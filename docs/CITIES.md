...
- **`population`**: **`int`** — **Phase 5.1.17a:** used only to cap **how many** non-center **`owned_tiles`** contribute **raw terrain** yield into **`CityYields.city_total_yield`** (**deterministic** selection in **`CityYields`**; assignments are **not** stored on **`City`**). **`FoundCity`** sets **`population`** to **`1`**; **`SetCityProduction`**, **`ProductionTick`**, and **`ProductionDelivery`** preserve **`population`** when rebuilding **`City`** rows. **No** population growth rules, food stockpile, specialists, housing, happiness, **city screen**, or **manual** worked-tile / citizen assignment UI in this slice (tests may construct other values).
...
**Phase 5.1.17a (shipped):** **`CityYields.city_total_yield`** sums (**1**) **`city_center_yield`**, (**2**) **`building_yield`** over **`building_ids`**, and (**3**) **`worked_tiles_yield`** — **raw terrain** only from up to **`population`** **non-center** hexes in **`owned_tiles`**, chosen **deterministically** by **`CityYields.worked_tiles_for_city`** (skip zero-yield terrain such as **WATER**). **`city_center_yield`** always counts the city tile separately (center rule); worked tiles never double-count the center hex. **Phase 5.1.17d:** **`CityYields.yield_breakdown_for_city`** returns those same pieces for inspection; **`CityProductionPanel`** shows a short read-only **Center / Buildings / Worked** line (**no** per-tile picker).
