...
- **`population`**: **`int`** — **Phase 5.1.17a:** caps **how many** non-center **`owned_tiles`** contribute **raw terrain** yield into **`CityYields.city_total_yield`**. **`FoundCity`** sets **`population`** to **`1`**; **`SetCityProduction`**, **`ProductionTick`**, **`ProductionDelivery`**, and **`SetCityWorkedTiles`** preserve **`population`** when rebuilding **`City`** rows. **Phase 5.1.18a:** optional **`City.manual_worked_tiles`** (see below) participates in that cap (**manual-first**, then deterministic auto-fill). **No** population growth rules, food stockpile, specialists, housing, happiness, or full **city screen** in this slice.
- **`manual_worked_tiles`**: **`Array`** (**`HexCoord`**) — **Phase 5.1.18a:** explicit worked-tile picks; **`[]`** means **all-auto** (same as **5.1.17a** behavior). Constructor-normalized (**owned**, non-center, deduped **`q,r`**). Mutated only via **`set_city_worked_tiles`** rebuilds (plus **`FoundCity`** seed **`[]`**).
- **`owned_tiles`**: **`Array[HexCoord]`** — **Phase 5.1.16g:** territory footprint; **center hex first**, then other owned cells. Default construction is **`[position]`** only. **`FoundCity`** assigns **center +** valid **radius-1** map hexes (including **WATER**); no overlap between cities; ring tiles already owned are skipped. **Not** culture/border growth — minimal substrate for **auto-worked** yields (**5.1.17a**). **`Scenario`** construction asserts every city owns its center, owned tiles lie on the map, and no tile is owned twice. **Prototype note (5.1.16g.2):** the **expanded** **g.1-lineage** **`make_prototype_play_map()`** (**island** with **full** outer **water**, **NE tongue**, **variegated** terrain, **smaller** **woods** patches) keeps **spatially separated** land anchors (see **`test_prototype_play_map_distribution.gd`**) so **`tile_already_owned`** / **coastal WATER** claims stay easy to exercise in headless tests.
...
**Phase 5.1.16g (shipped):** **`City.owned_tiles`** lists **`HexCoord`** cells the city controls (**center first**). **`FoundCity`** claims the **center** plus every **on-map** neighbor at hex distance **1** (including **WATER**); tiles already owned by another city are **skipped** on the ring, and founding on **any** owned tile is **`tile_already_owned`**. **`Scenario`** asserts **no** duplicate ownership, **all** owned tiles exist on the map, and each city **owns its center**. Read-only queries: **`tile_owner_city_id`**, **`city_owning_tile`**, **`tile_is_owned`**, **`tiles_owned_by_city`**. **`ProductionTick`**, **`ProductionDelivery`**, and **`SetCityProduction`** preserve **`owned_tiles`**.

**Phase 5.1.17a (shipped):** **`CityYields.city_total_yield`** sums (**1**) **`city_center_yield`**, (**2**) **`building_yield`** over **`building_ids`**, and (**3**) **`worked_tiles_yield`** — **raw terrain** only from up to **`population`** **non-center** hexes in **`owned_tiles`**, chosen by **`CityYields.worked_tiles_for_city`**. **Phase 5.1.18a:** valid **`City.manual_worked_tiles`** entries (still owned, non-center, nonzero raw terrain yield, deduped, capped by **`population`**) are applied **first** in order; remaining slots are filled from the **same deterministic auto ordering** as **5.1.17a**, skipping coords already used by manual picks. **`city_center_yield`** always counts the city tile separately (center rule); worked tiles never double-count the center hex. **Phase 5.1.17d:** **`CityYields.yield_breakdown_for_city`** returns those same pieces for inspection; **`CityProductionPanel`** shows a short read-only **Center / Buildings / Worked** line (**no** per-tile list beyond planning markers).

**Phase 5.1.16i (shipped, presentation):** **`CityTerritoryView`** draws a **selected-city-only** **territory** **border** (union perimeter, **two-tone** stroke, **no** interior fill) from **`tiles_owned_by_city`** (**[RENDERING.md](RENDERING.md)**) — **not** an all-cities overlay.
...
**Phase 5.1.16c (shipped):** **`CityYields`** ([city_yields.gd](../game/domain/city_yields.gd)) computes **v0** **Food** / **Production** / **Science** / **Coin** from **`HexMap`** terrain, an optional **`HexMap.has_woods`** overlay (prototype disk only today), **city-center** minimums (**Food** ≥ 2, **Production** ≥ 1 from center rule), and buildings (**`palace`**: **+1 Science**, **+1 Coin**, **no** extra **Production**). **5.1.17a:** **`city_total_yield`** also adds **raw terrain** from **auto-worked** non-center **`owned_tiles`** (bounded by **`population`**; deterministic order). **`ScienceTick`** consumes **`CityYields.science_for_player`** per **accepted** **end_turn** science step — not a flat **per-city** constant. **`produce_unit`** **production** remains the **`ProductionTick`** / **`ProductionDelivery`** path; **5.1.16d** **`ProductionTick`** advances **`progress`** by **`CityYields.city_total_yield`** **production** each tick (**founding location**, territory, terrain / woods / center rule affect pacing).
...
**Phase 5.1.4:** **[CityProductionPanel](../game/presentation/city_production_panel.gd)** is **presentation-only**: it **never** reads **`CityProjectDefinitions`** or **`EffectiveRules`**; production buttons and gating come **only** from filtering **`LegalActions.for_current_player(game_state)`** for **`set_city_production`** rows matching the selected **`city_id`**. Labels derive from **`project_id`** substrings (no registry display names). **Phase 5.1.16e:** the same panel shows **total city yields** (**Food** / **Production** / **Science** / **Coin**) from **`CityYields.city_total_yield`** for the selected city (read-only; **no** dense per-tile breakdown list in the hub). **5.1.17d:** a second line summarizes **`CityYields.yield_breakdown_for_city`** (**Center**, **Buildings**, **Worked**). **5.1.18a:** **Manage Citizens** / **PLANNING** map clicks submit **`set_city_worked_tiles`** via **`SelectionController`** (**[ACTIONS.md](ACTIONS.md)**) — embryo **toggle** for **one** manual worked hex vs auto, not a full assignment screen. **Phase 5.1.16f:** **`TileYieldOverlayView`** on the map shows per-hex **`CityYields`** (city totals vs **raw** terrain) when the **Yields** HUD toggle or **`KEY_Y`** is on (prototype inspection; not a new resource layer).

## Explicitly deferred

- **`ProduceUnit`** **player** action, economy/yields beyond this minimal completion rule.
- City **population growth**, **food stockpiling**, full **manual citizen / worked-tile assignment** UI (screens, drag-drop, multi-row pickers), **specialists**, **housing**, **happiness**, **amenities** (**5.1.17a** ships **`population`** **1** + deterministic auto-worked **`owned_tiles`**; **5.1.18a** ships a narrow **PLANNING** map toggle only); **garrison**, **zone of control** (culture / dynamic borders).
