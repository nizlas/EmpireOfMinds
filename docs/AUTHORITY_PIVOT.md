...
**One canonical gameplay core (locked, N8R 2026-08-08 — see [ARCHITECTURE_PRINCIPLES.md](ARCHITECTURE_PRINCIPLES.md)):** the no-adapter rule applies to **map models only** — the two match kinds must not share map models, and there is no adapter between `HexMap` and `WorldMap`. It is **never** a license to fork gameplay: map-independent gameplay rules (default city naming, founding validation/effect, production-selection validation/state, production tick/completion and delivery — eligibility, unit-id allocation, project clearing, engine-event construction — project progress and event representation, registries, turn/ID/event bookkeeping) have exactly ONE implementation in map-neutral pure modules (e.g. `server/app/domain/city_founding_rules.py`, `city_production_rules.py`, `combat_rules.py`, `turn_state.py`, `content/*`). The world modules (`world_actions.py`, `world_legal_actions.py`, `world_match.py`) are thin WorldMap authority/orchestration adapters over that core, not a second rules engine; rules trapped in deprecated `Scenario`/`HexMap` state are extracted, never copied. Keeping the deprecated runtime path operational is not a requirement.
