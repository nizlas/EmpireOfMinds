...
The **`WorldMap` match core** built by the N track ([PHASE_PLAN.md](PHASE_PLAN.md), slices N5–N9) is **born server-authoritative**: server-side `WorldMap` + content loading (N5), snapshot v3 + server-fed client bootstrap (N6), and units/movement/actions (N7) are implemented on the server from the start and never pass through a client-owned authority phase. Slices **B–D** established the **shared server-authoritative infrastructure** (server rules core, action validation/apply, snapshots, revisions, `state_hash`, event log) through the legacy loop; slices **E–G** concern the **legacy client cutover/migration**. At **N9**, only the **legacy-specific systems** are removed or migrated — `HexMap` (Python and GDScript), snapshot v2, legacy match creation, category-dependent rules, and the 2D renderer. The **`/v1` endpoint family, actions, revisions, snapshots, and `state_hash` remain the shared server foundation** for the `WorldMap` path.

**One canonical gameplay core (locked, N8R 2026-08-08 — see [ARCHITECTURE_PRINCIPLES.md](ARCHITECTURE_PRINCIPLES.md)):** the no-adapter rule applies to **map models only** — the two match kinds must not share map models, and there is no adapter between `HexMap` and `WorldMap`. It is **never** a license to fork gameplay: map-independent gameplay rules (default city naming, founding validation and the complete founding transition — founder consumption, city-id allocation/advancement, name, capital semantic, empty initial project — production-selection validation/state, production tick/completion and delivery — eligibility, unit-id allocation, project clearing, engine-event construction — project progress and event representation, registries, turn/ID/event bookkeeping) have exactly ONE implementation in map-neutral pure modules (e.g. `server/app/domain/city_founding_rules.py`, `city_production_rules.py`, `combat_rules.py`, `turn_state.py`, `content/*`). The world modules (`world_actions.py`, `world_legal_actions.py`, `world_match.py`) are thin WorldMap authority/orchestration adapters over that core, not a second rules engine; rules trapped in deprecated `Scenario`/`HexMap` state are extracted, never copied. Keeping the deprecated runtime path operational is not a requirement.
