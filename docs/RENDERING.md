...
## Phase 5.1.17k — Terrain edge blend (`TerrainEdgeBlendView`)

- **`TerrainEdgeBlendView`** ([terrain_edge_blend_view.gd](../game/presentation/terrain_edge_blend_view.gd)) — **`Node2D`**, **`z_index` 0**, **sibling after** **`MapView`**, **before** **`EmpireBorderView`** on [main.tscn](../game/main.tscn). **Reads** **`HexMap`**, **`HexLayout`**, **`MapCamera`** **only** — **no** `Scenario` field on node (**`TurnViewSync`** assigns **`map`** from **`Scenario.map`**).
- **Drawing (current):** **`draw_edge_blend`** export defaults **`false`** — **`_draw`** returns immediately so **no** semi-transparent edge ribbons (**hex-aligned banding** rollback). **`compute_blend_items`** remains for **`test_terrain_edge_blend_view.gd`** and future re-enable.
- **v1 scope (when enabled):** semi-transparent **filled quads** (**`draw_colored_polygon`**) along **shared** hex **edges** where one side is **`Terrain.PLAINS`** and the other **`Terrain.GRASSLAND`** (**canonical** undirected edge **once**; **`compute_blend_items`** sorted). **Excluded:** **WATER** / coast, **woods**/forest compositing, **shaders**/materials, painterly **texture** sampling, new **blend** PNGs, **`TerrainForegroundView`** edits, **`MapView`** rewrites.
...
- **Draw / input order** (**[main.tscn](../game/main.tscn)** under **`Main`**, map layers at **`MAP_LAYER_ORIGIN`**; pan = **`MapCamera.camera_world_offset`**; zoom = **`MapCamera.zoom`**):
  ...
  5. **[TerrainForegroundView](../game/presentation/terrain_foreground_view.gd)** — on **decorated** **PLAINS**: **4.6g** **raster** **front** clumps (**default**, **`forest_front_asset_opacity`**) or **4.6e**/**4.6f** **procedural** when **`use_forest_asset_overlays`** is **false**; **hex-owned** placement via **`anchor_pres`** / **`perspective_scale_at`**; **city** hexes **skip** **front** **raster** and use **procedural** **secondary**-only path (**4.6e**); **stable** with or without **units**; **`enable_unit_occlusion_test`** (**default** **false**) = **debug** **additive** occluder only (**4.6c**); **no** **`_input`**.
  6. **[SelectionController](../game/presentation/selection_controller.gd)** — hit-test / input only; **no** draw.
  7. **HUD** — **`TurnLabel`**, **`HudCanvas`** (**`CityProductionPanel`**), **`LogView`**, etc.
- **TFV symbol-grid depth merge (cities + units):** **`TerrainForegroundView`** may **delegate** **`CitiesView`** / **`UnitsView`** markers into **`_fg_draw_depth_merged_forest_symbol_grid_and_units`**. **Merge sort keys** for **`ty` 1 / 2** use **`cam.to_layout(cam.to_presentation(hex_center_world))`** — **not** **`city_effective_depth_presentation`** / **`unit_effective_depth_presentation`**. **5.1.15c** added a defensive same-hex **`ty`** ordering guard before **`sy`/`sx`** (**microfloat** safety); **manual play** already had **city-before-unit** when those keys tie. **`draw_city_marker_at`** / **`draw_unit_marker_at`** still use **anchors** + per-marker geometry; only **merge ordering** uses these keys. **5.1.15e:** when **`city_hex_has_units`**, **`CityNameplateView.draw_city_banner_on_canvas_item`** runs in this merge path **after** the city marker and **before** the unit marker; optional **`debug_log_shared_hex_marker_order`** logs **ty** 1/2 sequences per hex (default **off**); optional **`debug_log_shared_hex_layer_order`** logs **banner vs unit** steps for shared hexes (**off**).
...
- **4.6b-debug (visibility + wiring):** **`[main.gd](../game/main.gd)`** assigns **`TerrainForegroundView.camera = _map_camera`** — **same** **`MapCamera`** (**shared** **`MapPlaneProjection`**) as **`MapView`** / markers — so **pan** stays **aligned** (**foreground** must **not** rely on a one-off default **`MapPlaneProjection`** in **`_draw()`**). **`MapView.forest_back_opacity`** (default **0.85**) scales **procedural** **back** alpha only; **`MapView.forest_back_asset_opacity`** (**4.6g**, default **0.90**) scales **raster** **back** quads. **`TerrainForegroundView.forest_front_opacity`** scales **procedural** **front**; **`forest_front_asset_opacity`** (**default** **0.95**) scales **raster** **front**. **`MapView`** and **`TerrainForegroundView`** each expose **`use_forest_asset_overlays`** (**default** **true**); toggle **both** **off** in **inspector** to restore **full** **procedural** **4.6f**-style **fallback**. Optional **`TerrainForegroundView.forest_debug_log_counts_once`** prints **one** line — **off** by default.
- **4.6b-polish (silhouette read):** **`MapView`** **procedural** **path** draws **2–3** **canopy** **clusters** per decorated hex — **few** **overlapping** **large** circles plus an optional **skewed** **quad** in **presentation** space. **4.6g** **primary** **back** is **RGBA** **`res://assets/prototype/terrain/forest/forest_back_clump_0*.png`** (**`draw_texture_rect`**, **hex**-**anchored**, **`texture_filter` = **`LINEAR_WITH_MIPMAPS`** on **`MapView`**). **Foreground** **rasters**: **`forest_front_clump_0*.png`** in **`TerrainForegroundView`**. **Asset** salts **4100–4199** (**`cell_mix`**); **4.6e** salts **4000–4099** **unchanged** for **procedural**.
- **4.6c / 4.6d (occluder + draw order):** **`TerrainForegroundView`** holds **`Scenario`** (**read-only**) for **`cities_at`** (**4.6e** **main**-**skip**) and **optional** **`units_at`** (**debug** path **only**). **4.6d:** **hex-owned** foreground runs **first**; it **does** **not** depend on **unit** occupancy. **`enable_unit_occlusion_test`** (**default** **false**, **4.6e**): when **true**, **`_draw_unit_forest_occluder`** draws **after** as an **extra** **debug** overlay when **units** present and **no** **city** — **never** replaces **hex** clumps. **`unit_occluder_*`** / **`foreground_unit_reference_height_ratio`** apply **only** to that **test** path. **Not** **`Terrain.FOREST`**, **not** **rules**. **`SelectionController`** / **`EndTurnController`** / **`AITurnController`**: **`terrain_foreground_view`** sync (**picking** unchanged).
- **4.6e (hex-owned foreground composition — pivot-aware calibration):** For **decorated** **PLAINS**, **`_draw_plains_forest_front_hex_owned`** uses **`world_center` = `layout.hex_to_world(q,r)`**, **`anchor_pres` = `camera.to_presentation(world_center)`**, **`pscale` = `camera.perspective_scale_at(world_center)`**, **`base` = `HexLayout.SIZE × pscale`**. **All** sizes and **jitter** scale with **`base`**; **salts** **4000–4099** via **`PlainsForestScript.cell_mix`** only (**no** RNG). **Calibration:** **unit** **foot**/ **contact** **pivot** in **`UnitsView`** lands on **`anchor_pres`** ( **`unit_marker_pivot_y_ratio`** ~**0.90**, per-unit overrides) — **foreground** is **not** anchored to the **texture** **bottom** or the **extreme** **front** **hex** **edge**; **main** **center** sits **at** or **just** **above** **`anchor_pres`** (**`y_offset_main_ratio`** band) so **mass** can **overlap** **feet**/ **lower** **legs** while **head**/ **torso** stays readable. **Identical** art **with** or **without** a **unit** (no **unit** data in **normal** clump). **City** hex: **`scenario.cities_at(coord).size() > 0`** → **skip** **main** **clump** only; **secondary** still follows its **50%** **gate**. Muted **olive**/ **green** band; **4.6f** raises default **`forest_front_opacity`** and per-primitive **alpha** band (**circles** **∈** **[0.20, 0.36] × `forest_front_opacity`**, **polygon** **∈** **[0.22, 0.36] × `forest_front_opacity`**) so **shape** / **overlap** are easier to **judge** — **final** art may **lower** opacity again or **replace** procedural mass with **rasters** per **4.3j**. **`SelectionRingTopView`** **not** introduced; if **clutter** rises, lower **`forest_front_opacity`** slightly **in-editor** only.
- **Future:** **Further** raster polish / **true-alpha** audits (**4.3j**), **provenance**, and **optional** split of **terrain** nodes remain **steering**; **4.6g** already ships **prototype** forest **PNGs** (**visual-only** **PLAINS** gate **unchanged**).

## Phase 4.6g — Forest raster overlay assets (implemented)

- **Scope:** **Presentation-only** — **`MapView`** + **`TerrainForegroundView`**; **`res://assets/prototype/terrain/forest/*.png`** (**not** **`Terrain.FOREST`**, **not** rules). **Primary** draw: **textures** with **`use_forest_asset_overlays`** (**default** **true**); **fallback**: existing **procedural** code paths (**unchanged**). **Front** **city** hexes (**`scenario.cities_at(coord).size() > 0`**): **no** **front** **asset**; **procedural** **secondary**-only (**`skip_main`**). **Back** **assets** on **cities** **unchanged** (still drawn). **Hex-owned** placement: **`anchor_pres`**, **`base` = `HexLayout.SIZE × perspective_scale_at`**. **Imports**: **`mipmaps/generate=true`**, **`TEXTURE_FILTER_LINEAR_WITH_MIPMAPS`** on **both** views for **scaled** **draws**. New **checkout**: run **`godot --path game --import --headless`** once if **`.import`** **missing**.

## Phase 4.6e — Hex-owned forest foreground composition (implemented)

- **Scope:** **Presentation-only** — **`terrain_foreground_view.gd`** **foreground** grammar **only** (plus **`MapView.forest_back_opacity`** default **0.85**); **no** **`Terrain.FOREST`**, **no** domain/rules/picking/**`MapCamera`** math changes. **Hex-owned:** composition from **`q,r`** + **`world_center`** + shared **`MapCamera`** only; **no** **`units_at`** for **normal** clumps. **Pivot calibration:** **`anchor_pres`** aligns with **projected** **hex** **center** where **unit** **contact** **pivots** sit (**`UnitsView`** ratios **unchanged**); clumps sit **at/just above** that **anchor**, not the **marker** **texture** **bottom**. **City:** **main** skipped when **`scenario.cities_at(coord).size() > 0`**; **Secondary** may still draw. **Debug:** **`enable_unit_occlusion_test`** **default** **false**; **`_draw_unit_forest_occluder`** retained **only** for **tests**. **4.6f** ups default **`forest_front_opacity`** (**0.85**) and per-primitive **alpha** band for **evaluative** visibility (**below**). See **4.6e** bullets under **Phase 4.6b** for placement formulas.

## Phase 4.6f — Forest foreground visibility calibration (implemented)

- **Scope:** **Presentation-only** — no **geometry**, **salt**, **`forest_density_ratio`**, **city**-**skip**, or **branch** changes. **Goal:** make **hex-owned** clumps **easier** to **see** so **size**, **placement**, and **foot**/ **leg** **overlap** can be **judged** in **live** review; **back** **`forest_back_opacity`** left at **0.85**. **Future** art may **reduce** opacity again or **swap** to **PNG** overlays (**4.3j**).
...
- **`TurnLabel`** ([turn_label.gd](../game/presentation/turn_label.gd)) is a **`Label`** child of **`Main`** (renamed from the placeholder label in [main.tscn](../game/main.tscn)). Text is **`Turn N — Player P`** from **`compute_text(game_state)`**; **`main.gd`** assigns **`game_state`** and calls **`refresh()`** at startup. **`SelectionController`**, **`EndTurnController`**, **`CityProductionPanel`**, and **`AITurnController`** call **`refresh()`** after accepted actions so the HUD tracks **`TurnState`** (see [TURNS.md](TURNS.md)).

## Phase 5.1.4 — City production HUD (minimal)

- **`CityProductionPanel`** ([city_production_panel.gd](../game/presentation/city_production_panel.gd)) — **`PanelContainer`** under **`HudCanvas`** (**`CanvasLayer`**, raised **layer**, top-right **anchors** with **viewport-relative** margins in [main.tscn](../game/main.tscn)) so the **HUD** stays **on-screen** when the window/viewport size differs from the authored **2400×1500** baseline. **`refresh()`** sets **`MOUSE_FILTER_IGNORE`** while hidden so **map** **`_unhandled_input`** is not accidentally blocked; **`MOUSE_FILTER_STOP`** only when the panel is shown. **`compute_view_model`** / **`try_apply`** / controller **wiring** unchanged in intent (see **Phase 5.1.4** planning docs). **Phase 5.1.16e:** a compact **Yields:** line (**Food**, **Production**, **Science**, **Coin**) from read-only **`CityYields.city_total_yield(game_state.scenario, city)`** — **no** registry imports beyond **`LegalActions`** + **`CityYields`**; **no** per-tile breakdown yet.
- **`TileYieldOverlayView`** ([tile_yield_overlay_view.gd](../game/presentation/tile_yield_overlay_view.gd)) — **`Node2D`** map layer (**[main.tscn](../game/main.tscn)** **after** **`LightningTreeView`**, **before** **`CityWorkedTilesView`** and **`CityNameplateView`**, **`z_index` 1**): draws stable-order **CityYields** icons per **hex**, scaled with **`MapCamera.perspective_scale_at`** (not **`HudCanvas`**-fixed pixels). **`_configure_texture_filtering()`** sets **`CanvasItem.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS`**, matching **UnitsView** / **CitiesView** / **`TerrainForegroundView`**; **`yield_icons/*.png.import`** uses **`mipmaps/generate=true`** like **`map_markers/`** so scaled icons minify smoothly (no extra shaders or outline passes). **Phase 5.1.16f polish:** icon size uses roughly **2×** the first slice (**`YIELD_ICON_HEX_SIZE_RATIO` ≈ 0.36**, clamp **12–64** px in presentation space at current zoom) for readability; column/stack spacing remains proportional to **`icon_size`**. **City** hex: **`CityYields.city_total_yield`**; **non-city** land: **`raw_terrain_yield`**; **water** / all-zero → no draw. **Default OFF**; **`HudCanvas`** **`Yields`** **`CheckButton`** and **`KEY_Y`** toggle visibility in sync via **`YieldOverlayToggle`** ([yield_overlay_toggle.gd](../game/presentation/yield_overlay_toggle.gd)). Optional PNGs under **`res://assets/prototype/yield_icons/`**; fallback letters if missing.

## Phase 5.1.17h — Empire union border (`EmpireBorderView`)

- **`EmpireBorderView`** ([empire_border_view.gd](../game/presentation/empire_border_view.gd)) — **`Node2D`**, **`z_index` 0**, sibling **after** **`MapView`**, **before** **`CityTerritoryView`** in [main.tscn](../game/main.tscn). **Always-on** when **`scenario` / `layout` / `MapCamera`** are wired — **no** **`SelectionState`**, **no** **`CityYields`**. Builds each **`owner_id`**’s **union** of **`City.owned_tiles`** across **all** that owner’s cities; draws **outer** perimeter half-edges only (shared borders **between** same-owner tiles suppressed). Where **two owners** meet, **each** union treats the shared edge as **outer** (**both** draw). Reuses **`CityTerritoryView`** static perimeter helpers (**axial signature**, loop trace). **Visual:** **dual** **`Line2D`** rim (**owner-colored** **outer** + **indigo** **inner** inset), **same width / alpha recipe** as pre-correction **`CityTerritoryView`** territory emphasis (**perspective-scaled** stroke widths — readable at zoom). Appearance does **not** change when selecting or deselecting a city. **`TurnViewSync`** + **`SelectionController`** / **`EndTurnController`** / **`AITurnController`** assign **`scenario`** + **`queue_redraw`** like other map overlays.

## Phase 5.1.16i — Selected city territory outline (`CityTerritoryView`)

- **`CityTerritoryView`** ([city_territory_view.gd](../game/presentation/city_territory_view.gd)) — **`Node2D`**, **`z_index` 0**, sibling **after** **`EmpireBorderView`** on [main.tscn](../game/main.tscn). **`_draw`** is **dormant**: **no** selected-city **border-like** perimeter on the map — realm outline is **`EmpireBorderView`** **only** (**selection-independent**). **`Line2D`** pool + perimeter **static API** remain for **`EmpireBorderView`** topology reuse and **headless tests**. **Forward UX:** in **PLANNING**, tiles owned by the **selected** city read via **`CityWorkedTilesView`** **citizen** markers (**5.1.17j**): **`dim`** vs **`worked`** (**non-center** hexes); **swap** glyph later — **not** a second stroked rim from this node. Optional **debug** endpoint **`draw_circle`** caps only when legacy helpers run under debug flags. **Presentation-only**.

## Phase 5.1.17e — Selected-city citizen markers (`CityWorkedTilesView`; **5.1.17j** / **5.1.17j.1**)

- **`CityWorkedTilesView`** ([city_worked_tiles_view.gd](../game/presentation/city_worked_tiles_view.gd)) — **`Node2D`**, class/file name retained for wiring; **presentation-only** reads **`City.owned_tiles`**, **`CityYields.yield_breakdown_for_city`(..).`worked_tiles`** (**no** `worked_tiles_for_city` in this file). **`z_index` 1** in **[main.gd](../game/main.gd)**; sibling **after** **`TileYieldOverlayView`** — **Manage Citizens** citizen markers remain the **primary** planning layer and render **above** yield icons; **before** **`CityNameplateView`**. **Assets:** **`res://assets/prototype/map_markers/city_citizens/citizen_marker_dim.png`**, **`citizen_marker_worked.png`** — **`ResourceLoader.load`** as **`Texture2D`**. **Filter / import (prototype):** **`CanvasItem.TEXTURE_FILTER_LINEAR`** on this node (**no** mipmaps) — **`TEXTURE_FILTER_LINEAR_WITH_MIPMAPS`** **plus** **`mipmaps/generate=true`** on these PNGs **softened** edges and **bled** alpha at **map** scale (**ugly** **bands** / **halos** on **painterly** terrain); citizen **`.import`** files use **`mipmaps/generate=false`** (**distinct** from **`city_marker.png`** / **UnitsView** markers, which keep **mipmaps** for **zoom** **minification**). **`_draw`** (**and **`compute_draw_marker_items`**): **only** when **`CityViewState`** is **PLANNING** (**Manage Citizens**). **Citizen planning v0:** communicate **only** with those **marker** textures (**`dim`** = owned, **not** worked; **`worked`** = worked) — **no** large **full-hex** translucent **`draw_colored_polygon`** / alpha **tint** layer; overlapping **semi-transparent** hex **fills** show **edges** / **banding** against **painterly** **`MapView`** and are **not** used **here**. **Draw:** **`draw_texture_rect`**: anchor from **`camera.to_presentation(layout.hex_to_world(q,r))`**, size **`HexLayout.SIZE * 2 * citizen_icon_height_ratio * perspective_scale_at * planning_scale_mul`** (**`planning_scale_mul` 1.95** — **PLANNING** emphasis between the **1.85** and **2.2** tuning passes); center **y** is shifted by **`icon_side * planning_y_offset_icon_ratio`** (**default −0.24**, upward in screen space) so **terrain / yield** stay **more visible below**; markers may **slightly cross** the **hex** outline (**Civ-style**, readability over strict inset); **non-center** owned tiles only (**v0:** no marker on city center). **`kind`** **`dim`** vs **`worked`** from breakdown **`worked_tiles`** membership; **`compute_worked_marker_items`** returns the logical list for tests. Item order **`q`** asc, **`r`** asc. **`planning_marker_draw_style()`** applies **only** on the planning draw path. **Historical:** **5.1.17i** aquamarine inset polygons; **5.1.17j** replaced with citizen PNGs **only** (no return to **full-tile** translucent **fills** in **v0**).

**Layering direction (empire vs selection):** **[CITY_UX.md](CITY_UX.md)** — **`EmpireBorderView`** (**5.1.17h**, strength **5.1.17h.1**) draws the **only** visible **owner-union** realm perimeter (**merged** **`City.owned_tiles`** per **`owner_id`**); appearance **does not** change when selecting a city. **`CityTerritoryView`** does **not** draw a selected-city rim. **`CityWorkedTilesView`** draws **citizen** **`dim` / `worked`** markers **only** in **PLANNING**, **above** **`TileYieldOverlayView`** (yield icons) and **below** nameplates. None of these replace **culture** / **tile-swap** mechanics (not implemented).

## Phase 5.1.5 — City production panel polish (presentation-only)

- **Visual-only:** muted **parchment** **`StyleBoxFlat`** panel (**rounded** corners, subtle **border** / **shadow**), **section** **separators**, clearer **copy** (**City** header, **#id · Owner** subheader, **status** lines, **Train …** buttons, **Available production** + empty-state lines). **No** new textures/fonts; **`LegalActions`**-driven buttons unchanged in behavior.

## Phase 1.8 — AI turn trigger

- **`AITurnController`** ([ai_turn_controller.gd](../game/presentation/ai_turn_controller.gd)) is a **`Node2D`** **last** child of **`Main`** (after **`EndTurnController`**) in [main.tscn](../game/main.tscn): **`KEY_A`** (no echo) calls **`LegalActions.for_current_player`**, **`RuleBasedAIPlayer.decide`**, then **`game_state.try_apply`** once. On accept it **`selection.clear_unit()`**, re-points **`selection_view.scenario`** and **`units_view.scenario`**, **`queue_redraw()`**, **`turn_label.refresh()`**, **`log_view.refresh()`** when wired, **`city_production_panel.refresh()`** when wired. See [AI_LAYER.md](AI_LAYER.md), [ACTIONS.md](ACTIONS.md).

## Phase 1.9 — Action log debug view

- **`LogView`** ([log_view.gd](../game/presentation/log_view.gd)) is a **`Label`** child of **`Main`**. **Phase 4.4a:** **[main.tscn](../game/main.tscn)** places it in the **lower HUD band** (approx. **y 1220–1475**, **w ~1160**, **left 20** on the default **2400×1500** viewport) so it **does not** overlap the **hex map** (which sits around **`MAP_LAYER_ORIGIN`** **y ~428** with **~±320** vertical span for the tiny test map) or **`TurnLabel`** (**~y 20–100**). It shows the **last `MAX_ENTRIES` (10)** accepted log lines via **`compute_text(game_state)`**, **oldest at top, newest at bottom**; it reads only **`game_state.log.size()`** and **`game_state.log.get_entry(i)`** and **never mutates** domain state. **`main.gd`** assigns **`game_state`** and calls **`refresh()`** once at startup. **`SelectionController`**, **`EndTurnController`**, **`AITurnController`**, and **`CityProductionPanel`** (after its own accepted **`try_apply`**) call **`log_view.refresh()`** when **`game_state.log`** changes ( **`log_view`** is optional — **null-safe**). **`format_entry`** covers **`move_unit`**, **`end_turn`**, **`found_city`**, **`set_city_production`**, **`complete_progress`** (**Phase 5.1.6:** humanized **`progress_id`** + **` completed`**, optional extra lines **`       Unlocked: Train <Suffix>`** with **exactly seven leading spaces** per **`city_project` / `produce_unit:*`** row in **`unlocked_targets`**; other unlock rows omitted for clarity), engine **`production_progress`**, and engine **`unit_produced`**. A single log **entry** may span **multiple** text lines when **`complete_progress`** lists train unlocks. After **accepted** **`EndTurn`**, refreshed views reflect **delivered** units (**Phase 2.4c**). This is a **debug** surface only: **no** replay, **no** undo/redo, **no** polling or **`_process`** (see [ACTIONS.md](ACTIONS.md)).

## Phase 2.1 — City placeholder markers

- **`CitiesView`** ([cities_view.gd](../game/presentation/cities_view.gd)) is a **`Node2D`** sibling **after** **`MapView`** / **`EmpireBorderView`** / **`CityTerritoryView`** and **before** **`SelectionView`** in [main.tscn](../game/main.tscn). **Draw order among `z_index` 0 nodes:** **terrain → empire union border (dual rim, selection-independent) → (CityTerritoryView dormant slot) → cities → selection overlay → unit markers** (then **`z_index` 1** foreground / lightning / **`TileYieldOverlayView`** / **`CityWorkedTilesView`** planning markers; nameplates **`z_index` 2**). Cities are derived from **`Scenario.cities()`** via **`compute_marker_items`**. **Phase 4.3f:** when **`city_marker.png`** loads, **centered `draw_texture_rect` only** (no owner/dark rings). **Fallback:** **filled diamond** + outline. **Not** final art; see **[PHASE_4_3A_MARKER_SET.md](ASSET_REQUEST_PACKS/PHASE_4_3A_MARKER_SET.md)**.
...
## Phase 5.1.7 — Discovery unlock popup

- **`DiscoveryPopup`** ([discovery_popup.gd](../game/presentation/discovery_popup.gd)): **`PanelContainer`** child of **`HudCanvas`** (sibling of **`CityProductionPanel`**, **`DiscoveryActionPanel`**, **`SciencePanel`**, **`ScienceCompletedPopup`**, centered in [main.tscn](../game/main.tscn)); parchment-styled **Labels**, optional **`practical_line`** (e.g. **Controlled Fire** progress **+N**), **Unlocked:** bullet lines (**`Train …`**) for **`unlocked_targets`** rows matching **5.1.6** (**`city_project`** + **`produce_unit:`** prefix only) when **`action_type`** is **`complete_progress`**. **`compute_view_model(log_entry)`** is **untyped** so **`null`** and non-dict inputs yield a **hidden** view model; empty dicts, wrong action types, or (for **`complete_progress`**) **no** train unlocks also **hide**. **Phase 5.1.10:** **`action_type == science_bonus`** with **`bonus_id == lightning_scarred_tree`** shows curated **Lightning-Scarred Tree** copy and a **`practical_line`** from the event’s **`delta`** (**no** **`ProgressDefinitions`** import). **`maybe_show_for_log_index`** uses **`game_state.log.get_entry(index)`**; when **`compute_view_model`** is **hidden**, the panel’s prior state is left unchanged (**no** queue). **`SelectionController`** invokes it after **accepted** **`KEY_G`** / **`KEY_H`** via **`try_apply`**’s **`index`**. Phase **5.1.8b:** **`DiscoveryActionPanel`** invokes the same hook after **accepted** **Complete Discovery**. **`DiscoveryPopup.run_engine_popups_after_apply`**: after **MoveUnit** / **EndTurn** / **AI** **`try_apply`**, prefer the first new **`science_bonus`** for this popup; if the same new log slice also has **`science_completed`**, chain **`ScienceCompletedPopup`** on **OK** (see **5.1.9**). **`OK`** dismisses; **`MOUSE_FILTER_IGNORE`** when hidden, **`MOUSE_FILTER_STOP`** when visible.

## Phase 5.1.13 — Science selection panel (`SciencePanel`)

- **`SciencePanel`** ([science_panel.gd](../game/presentation/science_panel.gd)): **`HudCanvas`** **`PanelContainer`** under **`DiscoveryActionPanel`** (left column, **~268–540** px top); **always visible** during play so research is discoverable; parchment style aligned with **`CityProductionPanel`** / **`DiscoveryActionPanel`**.
- **Contents:** title **Science**; line for **Researching:** vs **Auto:** (effective target = explicit **`current_research_id`** when still **`ScienceAvailability.is_available`**, else first **`available_for`**); **progress** **`n / m`** from **`ProgressState.science_progress_for`** and **`ProgressDefinitions.cost`** for that effective id; heading **Available sciences** with one **Button** per available id (**✓** prefix when row is the explicit or auto **current**).
- **Input:** buttons call **`GameState.try_apply(SetCurrentResearch.make(...))`** only; on reject **`push_warning`** with reason; on accept refreshes **`TurnLabel`** / **`LogView`** when wired and **`call_deferred("refresh")`** on self. Domain **`ScienceTick`** is **not** invoked from the panel.
- **Refresh:** same cadence as **`CityProductionPanel`** / **`DiscoveryActionPanel`** from **`SelectionController`**, **`EndTurnController`**, **`AITurnController`**; **`DiscoveryActionPanel`** also queues **`SciencePanel`** refresh after accepted **Complete Discovery**.

## Phase 5.1.14 — SciencePanel locked-science hints

- **`SciencePanel`** adds heading **Locked sciences** when **`ScienceAvailability.locked_for`** is non-empty: **muted `Label` rows** (not buttons) with **`display`** text **`Name — Requires: A, B`** where **A, B** are **only missing** prerequisites (`ProgressDefinitions` order, **`science_display_name`** labels). Section **hidden** when there are no locked sciences. **Compact clip:** first **six** rows plus **`+N more locked sciences`** when the list is longer; full list remains in **`compute_view_model`**’s **`locked_rows`** for tests.

## Phase 5.1.9 — Science completed popup

- **`ScienceCompletedPopup`** ([science_completed_popup.gd](../game/presentation/science_completed_popup.gd)): **`HudCanvas`** **`PanelContainer`**; **`compute_view_model(log_entry)`** for **`action_type == science_completed`** with **`result == accepted`**. **Non-`controlled_fire`:** requires at least one **`city_project`** / **`produce_unit:*`** row in **`unlocked_targets`** (same **Train …** unlock block behavior as **`DiscoveryPopup`**). **`controlled_fire`:** **curated** narrative and practical line **without** Settler / **Train Settler** copy; **visible** even when **`unlocked_targets`** is **metadata-only** (no train rows). **No** **`ProgressDefinitions`** import. **`SelectionController`**, **`EndTurnController`**, and **`AITurnController`** use **`DiscoveryPopup.run_engine_popups_after_apply`** after accepted **`try_apply`**: when **no** new **`science_bonus`** is in the slice, **`show_first_new_science_completed`** shows the first new **`science_completed`**; when **`science_bonus`** is present, **`DiscoveryPopup`** runs first and may hand off to **`ScienceCompletedPopup`** after **OK**.

## Phase 5.1.11 — Unit nameplates (ownership banners)

- **`UnitNameplateView`** ([unit_nameplate_view.gd](../game/presentation/unit_nameplate_view.gd)): **`Node2D`** map-layer child of **`Main`**, **after** **`LightningTreeView`** in [main.tscn](../game/main.tscn), **`z_index` 2** in [main.gd](../game/main.gd) so labels draw **above** **`TerrainForegroundView`** unit markers and **`LightningTreeView`**, still **below** **`HudCanvas`** (**layer** 16). **`_draw`** only—**no** **`_input`**, **no** **`Control`** nodes, **no** mouse **pick** participation (selection stays on **`SelectionController`** / hex geometry). Uses **`MapCamera.to_presentation`** + **`HexLayout.hex_to_world`** like markers; vertical placement uses **`UnitsView.unit_marker_texture_rect_presentation`** when **`units_view`** is wired so the banner sits **above** the raster marker top. **Owner colors:** a **~25 px** (**22–28**) **left** vertical strip **inside** the **`StyleBoxFlat`** border (`owner_strip_width_px`), filled with muted teal (**player 0**), burgundy (**player 1**), fixed olive/mauve for **2–3**, deterministic **HSV** fallback for higher ids (distinct from **`UnitsView`** marker disk hues); unit label is **horizontally centered** in the parchment region to the **right** of the strip (padded, no overlap). **Labels:** **`UnitDefinitions.display_name`** when defined; else underscore humanization. **No** new image assets—**`StyleBoxFlat`**, **`draw_rect`**, **`draw_string`** (`ThemeDB.fallback_font`). Redraw: **`main._redraw_map_layers`**, **`SelectionController._sync_terrain_foreground_from_game_state`**, **`EndTurnController`** / **`AITurnController`** after accepted applies.

## Phase 5.1.15 — City name banners

- **`CityNameplateView`** ([city_nameplate_view.gd](../game/presentation/city_nameplate_view.gd)): **`Node2D`**, **`z_index` 2**, **`MAP_LAYER_ORIGIN`** in [main.gd](../game/main.gd). **5.1.15b:** in [main.tscn](../game/main.tscn) it appears **before** **`UnitNameplateView`** so **unit** nameplates paint **on top**. **5.1.15e:** when **`Scenario.units_at(city.position)`** is non-empty (**`city_hex_has_units`**), the parchment banner uses the **same above-marker placement** as normal cities (no large vertical offset). That banner is drawn by **`TerrainForegroundView`** immediately **after** **`draw_city_marker_at`** and **before** **`draw_unit_marker_at`** so the **unit** sprite can overlap/cover the city label; **`CityNameplateView`** skips drawing it when **`terrain_foreground_view`** is wired (**[main.gd](../game/main.gd)**). **`TerrainForegroundView`** (depth-merge / pass 2): **city** markers **before** **unit** markers on one hex (see **TFV** paragraph above). Optional **`debug_log_shared_hex_banner`** / **`EOM_DEBUG_SHARED_HEX_BANNER`**, **`TerrainForegroundView.debug_log_shared_hex_layer_order`** / **`EOM_DEBUG_SHARED_HEX_LAYER_ORDER`**. Same parchment **`StyleBoxFlat`** + owner strip as **`UnitNameplateView`**. **No** input / hit-testing. Redraw: **`main._redraw_map_layers`**, controllers, **`CityProductionPanel`**.

## Phase 5.1.8b — Lightning tree marker + discovery action panel

- **`LightningTreeView`** ([lightning_tree_view.gd](../game/presentation/lightning_tree_view.gd)): **`Node2D`** on **`Main`** **after** **`TerrainForegroundView`** in [main.tscn](../game/main.tscn) ([main.gd](../game/main.gd) sets **`z_index = 1`** on both) so the stump paints **above** the forest overlay (and thus above base **MapView** terrain). **Landmark scale (Phase 5.1.8c):** **`STUMP_HEIGHT_HEX_FRAC`** (~**0.5** × pointy-top hex height) keeps the stump a **small map marker**, not a full tree prop. **Placement:** prototype **`lightning_tree_hex`** is chosen as **open** **PLAINS** or **GRASSLAND** — a cell **not** listed in **`PlainsForestScript`**'s **`PROTOTYPE_FOREST_DECORATION_HEXES`** (see **`is_prototype_foreground_forest_hex`**) so **MapView** / **TerrainForegroundView** do not paint the **prototype forest-cluster** artwork on that tile (visual openness, not only base **HexMap** terrain). **`_draw`** uses **`MapCamera.to_presentation`**, **`HexLayout.hex_to_world`**, and a cached **`ImageTexture`** from **`scarred_tree_stump.png`** (**`Image.load`** — no committed **`.import`** required for headless). **Chroma:** only **bright screen magenta** (high **R**/**B**, low **G**); **no** global near-black key (avoids erasing bark). If keying would drop too much opacity, an **unkeyed** copy is used; if loading fails, a **fallback diamond** can draw at the tree hex (**`draw_fallback_shape_when_texture_missing`**). **`show_lightning_tree_debug_marker`** adds a ring for layout checks.
- **`DiscoveryActionPanel`** ([discovery_action_panel.gd](../game/presentation/discovery_action_panel.gd)): top-left **`HudCanvas`** panel (**parchment** **`StyleBoxFlat`** aligned with **`CityProductionPanel`**). **`compute_view_model`** uses **`ProgressCandidateFilter`** but **omits** **`controlled_fire`** (**Phase 5.1.9** — normal science completes via **`ScienceTick`**; panel reserved for future discoveries). **Hidden** when no remaining candidates. Refreshes in lockstep with **`CityProductionPanel`** via **`SelectionController._refresh_discovery_action_panel()`**, **`EndTurnController`**, and **`AITurnController`**.
