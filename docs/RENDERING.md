...
- **TFV symbol-grid depth merge (cities + units):** **`TerrainForegroundView`** may **delegate** **`CitiesView`** / **`UnitsView`** markers into **`_fg_draw_depth_merged_forest_symbol_grid_and_units`**. **Merge sort keys** for **`ty` 1 / 2** use **`cam.to_layout(cam.to_presentation(hex_center_world))`** only — **not** **`city_effective_depth_presentation`** / **`unit_effective_depth_presentation`**. Those **effective-depth** points differ on the **same hex** and previously let **cities** win **painter order**, **burying unit markers**. With **hex-center** keys, **same tile** ties **`sy`/`sx`** and **`ty`** applies (**city before unit**), so **units draw on top**. **`draw_city_marker_at`** / **`draw_unit_marker_at`** still use **anchors** and **per-marker** depth for **geometry**; only **merge ordering** changed.
