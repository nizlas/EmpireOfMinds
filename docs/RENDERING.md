...
- **`DiscoveryPopup`** ([discovery_popup.gd](../game/presentation/discovery_popup.gd)): **`PanelContainer`** child of **`HudCanvas`** (sibling of **`CityProductionPanel`**, centered in [main.tscn](../game/main.tscn)); parchment-styled **Labels**, **Unlocked:** bullet lines (**`Train …`**) for **`unlocked_targets`** rows matching **5.1.6** (**`city_project`** + **`produce_unit:`** prefix only). **`compute_view_model(log_entry)`** is **untyped** so **`null`** and non-dict inputs yield a **hidden** view model; empty dicts, non-**`complete_progress`** entries, or **no** train unlocks also **hide**. **`maybe_show_for_log_index`** uses **`game_state.log.get_entry(index)`**; when **`compute_view_model`** is **hidden**, the panel’s prior state is left unchanged (**no** queue). **`SelectionController`** invokes it after **accepted** **`KEY_G`** / **`KEY_H`** via **`try_apply`**’s **`index`**. **`OK`** dismisses; **`MOUSE_FILTER_IGNORE`** when hidden, **`MOUSE_FILTER_STOP`** when visible.
