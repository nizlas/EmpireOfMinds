...
- **`CityTerritoryView`** ([city_territory_view.gd](../game/presentation/city_territory_view.gd)) — **`Node2D`**, **`z_index` 0**, sibling **immediately after** **`MapView`** so the border reads as **painted on the base terrain**: **above** **MapView** ground tiles, **below** **CitiesView** / **UnitsView** / **`TerrainForegroundView`** (forest / raised art) / **`LightningTreeView`** / **`TileYieldOverlayView`** / nameplates (**`z_index` 2**). **Outline-only** (**no** interior tint / **no** `draw_colored_polygon`). **Perimeter topology:** **only** from **`Scenario.tiles_owned_by_city`** (duck-typed **q** / **r**) and **axial** **half-edges** toward **non-owned** neighbors. **Closed** **boundary** **loops** are **traced** in **hex/layout** space by **corner adjacency** (**`world_corner_key`** on **`HexLayout`** endpoints — **no** screen-space sort, **no** centroid angle order, **no** presentation-space loop assembly). **After** loop order is fixed, corners are **projected** with **`MapCamera`** and drawn as **closed** **`Line2D`** paths (**owner** **outer** + **thinner** **indigo** **inner**), **`LINE_JOINT_ROUND`**, **`LINE_CAP_NONE`** — **continuous** strokes **without** default **joint dots** / **rivets**. **Inner** loop: each **corner** offset **inward** using the **average** of the **two** **incident** edges’ **presentation** inward vectors toward their **owner** tile centers. Optional **debug** endpoint **`draw_circle`** caps only: **`debug_draw_territory_endpoint_caps`** / **`EOM_DEBUG_CITY_TERRITORY_CAPS`**. Per-frame **recompute** on **`queue_redraw`** (Line2D **points** + widths). **Presentation-only**; debug **`EOM_DEBUG_CITY_TERRITORY`** / **`EOM_DEBUG_CITY_TERRITORY_VISIBLE`** unchanged in intent.
...
**Layering direction (empire vs selection):** **[CITY_UX.md](CITY_UX.md)** — **always-on empire border** (future union perimeter per **`owner_id`** over **all** that faction’s city-owned tiles) is **orthogonal** to **selection-driven** overlays (**`CityTerritoryView`**, **`CityWorkedTilesView`**, future planning accents). Empire envelope must **not** be confused with worked-tile or single-city reads.
...
- **`CitiesView`** ([cities_view.gd](../game/presentation/cities_view.gd)) is a **`Node2D`** sibling **after** **`MapView`** / **`CityTerritoryView`** and **before** **`SelectionView`** in [main.tscn](../game/main.tscn). **Draw order among `z_index` 0 nodes:** **terrain → selected-city territory outline (border) → cities → selection overlay → unit markers** (then **`z_index` 1** foreground / lightning / yields; nameplates **`z_index` 2**). Cities are derived from **`Scenario.cities()`** via **`compute_marker_items`**. **Phase 4.3f:** when **`city_marker.png`** loads, **centered `draw_texture_rect` only** (no owner/dark rings). **Fallback:** **filled diamond** + outline. **Not** final art; see **[PHASE_4_3A_MARKER_SET.md](ASSET_REQUEST_PACKS/PHASE_4_3A_MARKER_SET.md)**.
