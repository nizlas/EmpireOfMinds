...
- **`CityWorkedTilesView`** ([city_worked_tiles_view.gd](../game/presentation/city_worked_tiles_view.gd)) — **`Node2D`** overlay (**read-only** **`CityYields.yield_breakdown_for_city`** **`.worked_tiles`**). **`z_index` 1** in **[main.gd](../game/main.gd)**; sibling **after** **`LightningTreeView`**, **before** **`TileYieldOverlayView`**. **Planning-only:** **`_draw`** is empty unless **`CityViewState.is_planning()`** (hub **Manage Citizens**); ordinary city selection shows **City Hub** **without** worked markers. **`planning_marker_draw_style()`** defines stroke/fill when markers draw. **Above** terrain / forest / lightning / **`z_index` 0** markers; **below** yield icons (**later** sibling) and nameplates (**`z_index` 2**).

**Layering direction (empire vs selection):** **[CITY_UX.md](CITY_UX.md)** — **`EmpireBorderView`** (**5.1.17h**, strength **5.1.17h.1**) draws the **only** visible **owner-union** realm perimeter in normal map modes (**merged** **`City.owned_tiles`** per **`owner_id`**). **`CityTerritoryView`** stays **dormant** (**no** rim / owned overlay yet — future deliberate slice). **`CityWorkedTilesView`** shows **worked-tile** markers **only** in **PLANNING** (**Manage Citizens**), **not** on ordinary city selection. None of these replace **culture** / **tile-swap** mechanics (not implemented).
