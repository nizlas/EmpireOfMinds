...
- **Draw / input order** (**[main.tscn](../game/main.tscn)** under **`Main`**, map layers at **`MAP_LAYER_ORIGIN`**; pan = **`MapCamera.camera_world_offset`**; zoom = **`MapCamera.zoom`**):
  ...
  7. **HUD** — **`TurnLabel`**, **`CityProductionPanel`**, **`LogView`**, etc.
...
- **`CityProductionPanel`** ([city_production_panel.gd](../game/presentation/city_production_panel.gd)) — **`VBoxContainer`** under **`Main`**, top-right band in [main.tscn](../game/main.tscn); **`mouse_filter = MOUSE_FILTER_STOP`** so **clicks** do **not** reach the map. **`compute_view_model(game_state, selection)`** shows **nothing** when **no** **`city_id`**, the **city** is **absent**, or when hiding; otherwise **status** from **`City.current_project`** and **buttons** from **`LegalActions.for_current_player`** filtered to **`set_city_production`** for that **city**. **`main.gd`** wires **`game_state`**, **`selection`**, **`cities_view`**, **`turn_label`**, **`log_view`**; **`SelectionController`** / **`EndTurnController`** / **`AITurnController`** hold **`city_production_panel`** and call **`refresh()`** after accepted **player** / **AI** / **`EndTurn`** actions (**`refresh()`** is **null-safe**). Buttons call **`game_state.try_apply`** directly (same responsibility pattern as **`SelectionController`**).
