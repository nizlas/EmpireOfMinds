...
- **`CityTerritoryView`** ([city_territory_view.gd](../game/presentation/city_territory_view.gd)) — **`Node2D`**, **`z_index` 0**, sibling **after** **`EmpireBorderView`** on [main.tscn](../game/main.tscn). **5.1.17h correction:** **`_draw`** is **dormant** in normal play (**no** visible rim from this node while **`EmpireBorderView`** owns the realm outline); **`Line2D`** pool + perimeter **static API** remain for **`EmpireBorderView`** reuse and future **CityPlanningMode** city-centric emphasis. When planning mode ships, intended recipe matches historical territory rim (**owner** **outer** + **indigo** **inner**, **`LINE_JOINT_ROUND`**, **`LINE_CAP_NONE`**). Optional **debug** endpoint **`draw_circle`** caps only: **`debug_draw_territory_endpoint_caps`** / **`EOM_DEBUG_CITY_TERRITORY_CAPS`**. **Presentation-only**; debug **`EOM_DEBUG_CITY_TERRITORY`** unchanged in intent.

## Phase 5.1.17e — Selected-city worked-tile overlay (`CityWorkedTilesView`)

- **`CityWorkedTilesView`** ([city_worked_tiles_view.gd](../game/presentation/city_worked_tiles_view.gd)) — **`Node2D`** **selected-city** informational overlay (read-only **`CityYields.yield_breakdown_for_city`** **`.worked_tiles`**). **`z_index` 1** set in **[main.gd](../game/main.gd)**; **[main.tscn](../game/main.tscn)** sibling **after** **`LightningTreeView`**, **before** **`TileYieldOverlayView`**. **Intentionally draws above** base terrain, **forest** **`TerrainForegroundView`**, **`LightningTreeView`**, and **`z_index` 0** city/unit markers **so** worked tiles **read** when **focusing** the **selected** city; **still below** **`TileYieldOverlayView`** (**same** **`z_index`**, **later** sibling → **yield** **icons** **win**) **and below** nameplates (**`z_index` 2**).

**Layering direction (empire vs selection):** **[CITY_UX.md](CITY_UX.md)** — **`EmpireBorderView`** (**5.1.17h**, strength **5.1.17h.1**) draws the **only** visible **owner-union** realm perimeter in normal map modes (**merged** **`City.owned_tiles`** per **`owner_id`**). **`CityTerritoryView`** does **not** add a second rim until **CityPlanningMode** (slot retained above **`EmpireBorderView`**). **`CityWorkedTilesView`** remains **selected-city** worked-tile markers only. None of these replace **culture** / **tile-swap** mechanics (not implemented).
