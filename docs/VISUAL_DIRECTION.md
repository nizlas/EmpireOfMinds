...
- **Phase 4.6g** adds **primary** **raster** forest **clumps** (**`res://assets/prototype/terrain/forest/`**) on **decorated** **PLAINS** — **back** in **`MapView`**, **front** in **`TerrainForegroundView`** — **hex**-**owned** / **`anchor_pres`** **calibrated**; **`use_forest_asset_overlays = false`** restores **procedural** **only**. Still **not** **`Terrain.FOREST`** / **not** domain **rules**.
- **Phase 4.6e** ships **larger**, **muted**, **hex-owned** **foreground** **masses** in **`TerrainForegroundView`**, **pscale**-scaled and **calibrated** to the **projected** **hex** **center** / **unit** **foot-contact** **line** (not **sprite-bottom**). **Unit-aware** **`_draw_unit_forest_occluder`** is **demoted** to **debug** only (**`enable_unit_occlusion_test`** default **off**).
...
- **Layering:** **Back** forest in **`MapView`** (**raster** **default** **4.6g**, or **procedural**) **behind** **units** / **most** **city** markers; **`TerrainForegroundView`** **front** **after** **`UnitsView`** — overlaps **feet**/ **lower** **legs**; **upper** **body** stays **readable**.
