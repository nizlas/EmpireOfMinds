...
The opt-in **`world_map`** match path has its own server-side action set in `server/app/domain/world_actions.py`, deliberately separate from the frozen legacy modules below (no `Scenario`, no `HexMap`, no adapter). N7a supports **`move_unit`** and **`end_turn`**, **N7g.1 (2026-08-07)** added **`attack_unit`**, and **N8a (2026-08)** added **`found_city`** (same wire shapes and `schema_version` `1` as the legacy actions). Known legacy action type **`set_city_production`** still rejects with **`unsupported_action_for_match_kind`** until N8b; unrecognized types reject with **`unknown_action_type`**. World `move_unit` legality comes exclusively from the authoritative `WorldMap` (smooth edges permit, cliffs block, occupancy blocks, `has_attacked` blocks — [MOVEMENT_RULES.md](MOVEMENT_RULES.md)); world `attack_unit` reuses the shared pure Local Combat 0.1 core (warrior-vs-warrior over a smooth edge, retaliation/elimination server-side — [CLOUD_API_V0.md](CLOUD_API_V0.md)); world `found_city` consumes an eligible settler and appends a minimal snapshot city from `next_city_id` (cities never block movement; tile-already-has-city is the only city-placement restriction); world `end_turn` advances `turn_state` and clears every `has_attacked` flag, with **no** production/food/science ticks, delivery, or movement refresh (planned N8c processing changes this on the world path only — see below). Full wire contract, gate order, literal rejection strings, and fail-closed map-identity verification: [CLOUD_API_V0.md](CLOUD_API_V0.md). Clients never validate world actions locally; the served `legal-actions` list (N7b, implemented) is the only legality source clients consume.

**World action growth ([PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”):** **`found_city`** landed in **N8a** (settler consumed; additive snapshot `cities` + `next_city_id`; locked: an existing city on the founding tile is the only city-placement restriction; manual/visual gate PENDING); **`set_city_production`** remains planned for **N8b** (flat yields v2; **no unlock gating — locked 2026-08-06**, both `produce_unit` projects always selectable). Production completion/spawn is **engine-side `end_turn` processing** (**N8c**, locked legacy-equivalent timing: tick the ending player's production, advance the turn, deliver for the player who has just become current; event order `production_progress`* → `end_turn` → `unit_produced`*), never a player action — mirroring the legacy rule that `unit_produced` is engine-generated. Until N8b lands, `set_city_production` keeps `unsupported_action_for_match_kind`.
