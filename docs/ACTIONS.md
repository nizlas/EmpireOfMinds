...
## World actions (N7a — server-authoritative, `world_map` matches)

The opt-in **`world_map`** match path has its own server-side action set in `server/app/domain/world_actions.py` (thin adapters over the canonical N8R core — no private founding/production algorithms; no `Scenario`/`HexMap` imports on the world path). N7a supports **`move_unit`** and **`end_turn`**, **N7g.1 (2026-08-07)** added **`attack_unit`**, **N8a (2026-08; gate PASS 2026-08-08)** added **`found_city`** (same wire shapes and `schema_version` `1` as the legacy actions), and **N8b (2026-08; gate PASS 2026-08-08)** added **`set_city_production`** (`schema_version` `2`). Unrecognized types reject with **`unknown_action_type`**. World `move_unit` legality comes exclusively from the authoritative `WorldMap` (smooth edges permit, cliffs block, occupancy blocks, `has_attacked` blocks — [MOVEMENT_RULES.md](MOVEMENT_RULES.md)); world `attack_unit` reuses the shared pure Local Combat 0.1 core (warrior-vs-warrior over a smooth edge, retaliation/elimination server-side — [CLOUD_API_V0.md](CLOUD_API_V0.md)); world `found_city` consumes an eligible settler and appends a minimal snapshot city from `next_city_id` (cities never block movement; tile-already-has-city is the only city-placement restriction); world `set_city_production` sets or clears city `current_project` with **no unlock gating**; world `end_turn` advances `turn_state` and clears every `has_attacked` flag, with **no** production/food/science ticks, delivery, or movement refresh until **N8c** (unblocked; not yet implemented — see below). Full wire contract, gate order, literal rejection strings, and fail-closed map-identity verification: [CLOUD_API_V0.md](CLOUD_API_V0.md). Clients never validate world actions locally; the served `legal-actions` list (N7b, implemented) is the only legality source clients consume.

**World action growth ([PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”):** **`found_city`** landed in **N8a** (settler consumed; additive snapshot `cities` + `next_city_id`; locked: an existing city on the founding tile is the only city-placement restriction; **manual/visual gate PASS 2026-08-08**); **`set_city_production`** landed in **N8b** (flat yields v2 constant established; **no unlock gating — locked 2026-08-06**, both `produce_unit` projects always selectable; **manual/visual gate PASS 2026-08-08**). Production completion/spawn is **engine-side `end_turn` processing** (**N8c** Done; manual/two-client runtime gate PENDING; unblocked by N8R review PASS at `709810c`; locked legacy-equivalent timing: tick the ending player's production, advance the turn, deliver for the player who has just become current; event order `production_progress`* → `end_turn` → `unit_produced`*), never a player action — mirroring the legacy rule that `unit_produced` is engine-generated.

Everything below is the **legacy (Godot local + Cloud v2) action inventory**, frozen until its retirement in N9.
...
4. **Phase 5.1.18a:** for each **current-player** city, deterministic **`set_city_worked_tiles`** actions: one per **eligible** **non-center** **owned** tile with **nonzero** **`CityYields.raw_terrain_yield`**, ordered **`q`** ascending then **`r`** ascending; plus **`tiles: []`** **clear-to-auto** when **`city.manual_worked_tiles`** is non-empty. Entries are **omitted** when **`SetCityWorkedTiles.validate`** fails (e.g. **`assignment_unchanged`** for the tile already manual).
5. Exactly one **`EndTurn.make(current_player_id)`**, **last**.
...
**Cloud / Slice C7–C10:** the HTTP authority exposes **`GET /v1/matches/{match_id}/legal-actions`** (see [CLOUD_API_V0.md](CLOUD_API_V0.md)) returning **submit-ready** **`Dictionary`**-shaped JSON for **`end_turn`**, **`move_unit`**, **`found_city`**, **`set_city_production`**, and **`attack_unit`** — no new authority semantics; it reuses the same validators as **`POST .../actions`**.
...
**Server (Cloud / Slice C10):** `POST /v1/matches/{id}/actions` accepts **`attack_unit`** with the same fields as **`AttackUnit.make`**: **`schema_version` `1`**, **`action_type`**, **`actor_id`**, **`attacker_id`**, **`defender_id`** only (no **`from`** / **`to`**). Server **`combat_rules.resolve_attack`** mirrors Godot **`CombatRules.resolve_attack`**; **`attack_unit.apply_with_result`** sets attacker **`remaining_movement = 0`**. Accepted events include the combat snapshot fields above (see [CLOUD_API_V0.md](CLOUD_API_V0.md)). Selection-mode **`GET .../legal-actions`** lists adjacent enemy **Warrior** attacks for the selected unit; summary mode omits per-unit attacks.
...
## SetCityWorkedTiles schema (Phase 5.1.18a)

| Field | Type | Meaning |
|--------|------|--------|
| `schema_version` | `int` | `1` (`SetCityWorkedTiles.SCHEMA_VERSION`). |
| `action_type` | `String` | `"set_city_worked_tiles"`. |
| `actor_id` | `int` | Must match **`city.owner_id`** in **`validate`**; must equal **`turn_state.current_player_id()`** at **`try_apply`**. |
| `city_id` | `int` | Target city. |
| `tiles` | `Array` | Each element **`[q, r]`** (`int` pair). **`[]`** clears manual assignment (**all-auto**). Count must be **`<= city.population`**. |

Built with **`SetCityWorkedTiles.make(actor_id, city_id, tiles)`** in [set_city_worked_tiles.gd](../game/domain/actions/set_city_worked_tiles.gd). **`apply`** rebuilds the target **`City`** with normalized **`manual_worked_tiles`** (fresh **`HexCoord`** rows).

### SetCityWorkedTiles validation order

**`SetCityWorkedTiles.validate(scenario, action)`** (does **not** check **`current_player_id`**):

1. `scenario_null`
2. `wrong_action_type`
3. `unsupported_schema_version`
4. `malformed_action` — **`actor_id`**, **`city_id`**, **`tiles`** (**must** be **`Array`**; each tile **`[q,r]`** with **`TYPE_INT`** coords)
5. `unknown_city`
6. `actor_not_owner`
7. `too_many_tiles` — **`tiles.size() > city.population`**
8. Per payload tile in order: `malformed_action` (inner shape), `duplicate_tile`, `tile_is_center`, `tile_not_owned`, `tile_zero_yield`
9. `assignment_unchanged`

### SetCityWorkedTiles and GameState.try_apply

After the **common** **`actor_id`** / **`current_player`** gate: **`validate` → `apply` →** log entry includes **`city_id`**, **`tiles`** (array of **`[q,r]`** int pairs), **`result: "accepted"`**.
...
- After an **accepted** **`CompleteProgress`** via **`KEY_G`**, **`SelectionController`** calls **`turn_label.refresh()`** / **`log_view.refresh()`** when wired only; **does not** re-point **`scenario`**, **does not** **`queue_redraw()`** map/city/unit/selection views, **does not** change **selection**.
- After an **accepted** **`CompleteProgress`** via **`KEY_H`** (detector candidate), the **same** refresh rules apply as **`KEY_G`**.
