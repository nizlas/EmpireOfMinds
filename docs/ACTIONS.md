...
**Planned world action growth ([PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”):** **`attack_unit`** gains its world path in **N7g** (exact Local Combat 0.1 math parity; additive unit `current_hp` + per-turn attacked flag — proposed contracts pending review); **`found_city`** in **N8a** (settler consumed; additive snapshot `cities` + `next_city_id`); **`set_city_production`** in **N8b** (flat yields v2, no unlock gating — proposed). Production completion/spawn is **engine-side `end_turn` processing** (**N8c**), never a player action — mirroring the legacy rule that `unit_produced` is engine-generated. Until each slice lands, the rejections above remain the implemented behavior.
