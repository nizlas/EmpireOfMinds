...
**World action growth ([PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”):** **`found_city`** landed in **N8a** (settler consumed; additive snapshot `cities` + `next_city_id`; locked: an existing city on the founding tile is the only city-placement restriction; **manual/visual gate PASS 2026-08-08**); **`set_city_production`** landed in **N8b** (flat yields v2 constant established; **no unlock gating — locked 2026-08-06**, both `produce_unit` projects always selectable; **manual/visual gate PASS 2026-08-08**). Production completion/spawn is **engine-side `end_turn` processing** (**N8c**, unblocked by N8R review PASS at `709810c`; locked legacy-equivalent timing: tick the ending player's production, advance the turn, deliver for the player who has just become current; event order `production_progress`* → `end_turn` → `unit_produced`*), never a player action — mirroring the legacy rule that `unit_produced` is engine-generated.
