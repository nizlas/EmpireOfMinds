...
On the opt-in **`world_map`** match path, units are **server-owned minimal state** in snapshot v3 (`server/app/domain/world_match.py`), separate from the legacy Godot `Unit`/`Scenario` types below: each unit is exactly `{"id": int, "owner_id": int, "position": [q, r], "type_id": "settler" | "warrior"}`, deterministically sorted ascending by `id`. World units carry **no** movement points, moved flag, or HP in N7 — world movement v1 has no movement budget ([MOVEMENT_RULES.md](MOVEMENT_RULES.md)) and combat is out of N7 scope.

**Starting units** come from the map-keyed spawn table in `server/app/domain/world_scenario.py` (validated against the canonical `WorldMap` at creation; divergence fails creation with no partial match). World creation requires exactly two `player_ids` (arbitrary integers); the first entry owns units 1–2 (settler, warrior), the second owns units 3–4. For `handdrawn_test_map_full_01`: unit 1 settler `[1,1]`, unit 2 warrior `[2,1]` (first player); unit 3 settler `[2,14]`, unit 4 warrior `[2,13]` (second player). Wire contract: [CLOUD_API_V0.md](CLOUD_API_V0.md).
