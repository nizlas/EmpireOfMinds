...
On the opt-in **`world_map`** match path, units are **server-owned minimal state** in snapshot v3 (`server/app/domain/world_match.py`), separate from the legacy Godot `Unit`/`Scenario` types below: each unit is exactly `{"id": int, "owner_id": int, "position": [q, r], "type_id": "settler" | "warrior"}`, deterministically sorted ascending by `id`. World units carry **no** movement points or moved flag — world movement v1 has no movement budget ([MOVEMENT_RULES.md](MOVEMENT_RULES.md)) — and **no HP yet**: world Combat 0.1 is the planned **N7g** slice, which adds additive `current_hp` plus a per-turn attacked flag (proposed contracts pending review — [PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”).
