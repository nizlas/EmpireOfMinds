...
- **Locked principles:** see [ARCHITECTURE_PRINCIPLES.md](ARCHITECTURE_PRINCIPLES.md) "One canonical gameplay core" — adapters may differ, gameplay rules may not; extract, never copy; no match-kind-specific copies; the deprecated runtime path is not a requirement. **N8c is unblocked** by the N8R review PASS and may proceed as its own slice (not implemented by N8R).

#### N8c — Authoritative production processing + deterministic spawn

- **Objective:** production advances through authoritative turn processing and completed units spawn deterministically into the continuing loop — target-loop points 7–8. Prereqs: **N8b and N7g** (produced units carry the N7g `current_hp` and `has_attacked` fields); **unblocked** (N8R architectural review PASS at `709810c`, 2026-08-08) — the canonical tick/delivery loop already exists (`city_production_rules.tick_production` / `deliver_completed_production`, N8R): N8c supplies the flat yields, the WorldMap placement policy (as delivery facts/inputs — a city with no legal placement is deferred by the adapter, per the locked rules below), and the snapshot-v3 materialization; it must never copy the loop.
