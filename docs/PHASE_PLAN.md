...
- **Status: implemented 2026-08; manual/visual gate PENDING** (do not mark the gate passed until Niclas approves city marker placement/readability and shared-tile selection feedback).
...
- **Manual/visual gate: PENDING.** City marker placement/readability **and** shared-tile selection feedback (unit-vs-city cycling is legible) still require Niclas approval.
...
- **Status:** implemented 2026-08; **manual/visual gate PENDING** (stacked on pending N8a).
...
- **Status:** implemented 2026-08-08 and corrected the same day after architectural review (the first pass had only extracted the WorldMap-side rules, leaving the proven legacy algorithms as parallel implementations); **renewed architectural review PENDING** (stacked on pending N8a/N8b; no new manual/visual behavior — the N8a/N8b gates carry over unchanged).
...
- **Locked principles:** see [ARCHITECTURE_PRINCIPLES.md](ARCHITECTURE_PRINCIPLES.md) "One canonical gameplay core" — adapters may differ, gameplay rules may not; extract, never copy; no match-kind-specific copies; the deprecated runtime path is not a requirement. **N8c is blocked until N8R has been reviewed.**

#### N8c — Authoritative production processing + deterministic spawn

- **Objective:** production advances through authoritative turn processing and completed units spawn deterministically into the continuing loop — target-loop points 7–8. Prereqs: **N8b and N7g** (produced units carry the N7g `current_hp` and `has_attacked` fields); **blocked until N8R has been reviewed** — the canonical tick/delivery loop already exists (`city_production_rules.tick_production` / `deliver_completed_production`, N8R): N8c supplies the flat yields, the WorldMap placement policy (as delivery facts/inputs — a city with no legal placement is deferred by the adapter, per the locked rules below), and the snapshot-v3 materialization; it must never copy the loop.
