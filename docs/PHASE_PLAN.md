...
| **N7g** | **In progress — N7g.1 server authority, N7g.2 served attack legality, and N7g.3 client combat implemented (2026-08-07); manual combat gate PENDING** | World Combat 0.1: server-authoritative `attack_unit` on world matches with exact Local Combat 0.1 math parity — **warrior-vs-warrior only (locked)**; **additive** snapshot v3 unit `current_hp` + per-turn attacked flag; served attack rows; minimal client feedback — contracts locked by review 2026-08-06 (below). **N7g.1 (implemented):** snapshot combat fields, world `attack_unit` validation/resolution/persistence via the shared pure Local Combat 0.1 core, `unit_already_attacked` move gating, and the end-turn `has_attacked` reset ([CLOUD_API_V0.md](CLOUD_API_V0.md)). **N7g.2 (implemented):** served `attack_unit` legality — selected owned warriors return submit-ready attack rows (before the move rows, canonical `DIRECTIONS` order) derived through the N7g.1 validators; summaries count attacks + moves. **N7g.3 (implemented 2026-08-07; third visual-correction pass in progress):** client combat — distinct attack markers, exact served-row submission, minimal HP/has_attacked status, melee approach (`MELEE_STANDOFF_DISTANCE` 0.80 confirmed), impact-timed attack/hit overlap with fatal `Dead` starting directly at impact (no `Hit_Reaction_1` prepend), centralized `ANIM_BLEND_DEFAULT_SEC` 0.28 clip transitions, shortest-arc survivor travel-facing blend (`TRAVEL_FACING_BLEND_SEC` 0.28), continuous corpse terrain support during `Dead`, snapshot-authoritative return-or-capture travel; server capture rule when the defender is eliminated. N7g stays incomplete until Niclas passes the local manual/two-client combat gate ([VALIDATION_CHECKLIST.md](VALIDATION_CHECKLIST.md)) |
...
- **Client work (N7g.3, implemented 2026-08-07; third visual-correction pass — manual gate PENDING):** attack-target markers from served rows (visually distinct from move markers in color and shape), submission of exact served rows, HP + `has_attacked` shown minimally (selection/status line), and **character combat presentation**: presentation-only melee approach (`Walking` to `MELEE_STANDOFF_DISTANCE` 0.80 in front of the defender; never emits `unit_arrived`), impact-timed overlapping attack with non-fatal `Hit_Reaction_1` or fatal `Dead` starting directly at `COMBAT_IMPACT_DELAY_SEC` ≈ 0.5 s (never prepend `Hit_Reaction_1` before fatal `Dead`; same for retaliation), every semantic clip change crossfades via centralized `ANIM_BLEND_DEFAULT_SEC` 0.28 (legacy map-view `idle_end_blend_sec`), survivor travel-facing uses shortest-arc `TRAVEL_FACING_BLEND_SEC` 0.28 (N7f locomotion still snaps facing — combat return cannot), retaliation only when the event says `retaliated == true` (same impact overlap; never after defender death), **continuous corpse terrain support while `Dead` plays** (animated head/hips/feet contact → minimum lift + multi-point pitch/roll; no post-clip pop), and survivor traversal to the **snapshot-authoritative** final anchor (return home if the defender survives; occupy the captured tile if the deferred snapshot places the attacker there). Living units stay upright yaw-only; corpses are the explicit pitch/roll exception. Leg grounders pause around authored combat clips. Gameplay input + served legality stay blocked through approach, combat, death, and final traversal until the deferred snapshot applies and fresh legality is refetched. Outcomes come only from the accepted server event and snapshot — never recomputed or inferred from animation; `CombatClashBurstView` remains excluded.
