...
| **N7g** | Planned | World Combat 0.1: server-authoritative `attack_unit` on world matches with exact Local Combat 0.1 math parity — **warrior-vs-warrior only (locked)**; **additive** snapshot v3 unit `current_hp` + per-turn attacked flag; served attack rows; minimal client feedback — contracts locked by review 2026-08-06 (below) |
...
#### Gameplay contracts — locked vs still proposed

The plan deliberately does **not** silently canonize new gameplay rules; everything not listed here reuses existing canonical contracts (Local Combat 0.1 math, legacy wire shapes, project registry, city naming, event names).

**Locked by review (2026-08-06):**
...
8. **N7d served-legality freshness:** legal-actions results are valid only for their returned revision and still-current selection; stale asynchronous responses discarded; newer snapshots clear markers/actions and refetch while the selection remains valid; a served row is never submitted when its legal-actions revision differs from the held snapshot revision.

**Still proposed (approve or amend before the owning slice is implemented):**

1. **N8a city passability & spacing:** cities never block unit movement; no minimum distance between cities; founding rejects only a tile that already has a city.
2. **N8a shared-tile selection precedence:** first pick selects the unit, repeated pick on the same tile cycles to the city (presentation-only; the N8a manual gate reviews its feedback legibility).
3. **N8b no unlock gating / no world science in N8:** both `produce_unit` projects always selectable; the world path carries no `progress_state`. Science/research on the world path is a post-N8 decision.
4. **N8b flat yield value:** flat base production **1 per city per turn** (with registry costs 2/2 → a unit every second turn). Balance placeholder, trivially retunable.
