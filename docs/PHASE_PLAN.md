...
| **N7g** | **In progress — N7g.1 server authority, N7g.2 served attack legality, and N7g.3 client combat implemented (2026-08-07); manual combat gate PENDING** | World Combat 0.1: server-authoritative `attack_unit` on world matches with exact Local Combat 0.1 math parity — **warrior-vs-warrior only (locked)**; **additive** snapshot v3 unit `current_hp` + per-turn attacked flag; served attack rows; minimal client feedback — contracts locked by review 2026-08-06 (below). **N7g.1 (implemented):** snapshot combat fields, world `attack_unit` validation/resolution/persistence via the shared pure Local Combat 0.1 core, `unit_already_attacked` move gating, and the end-turn `has_attacked` reset ([CLOUD_API_V0.md](CLOUD_API_V0.md)). **N7g.2 (implemented):** served `attack_unit` legality — selected owned warriors return submit-ready attack rows (before the move rows, canonical `DIRECTIONS` order) derived through the N7g.1 validators; summaries count attacks + moves. **N7g.3 (implemented 2026-08-07; fourth visual-correction pass in progress):** client combat — distinct attack markers, exact served-row submission, minimal HP/has_attacked status, melee approach (`MELEE_STANDOFF_DISTANCE` 0.80), impact-timed attack/hit with fatal `Dead` at impact, centralized clip blend 0.28, centralized ordinary facing blend (`FACING_BLEND_SEC` 0.28), combat-support planted-foot grounding during living one-shots (Walking grounding preserved; living ModelRoot upright), localized Spine02 elevation aim, continuous corpse support during `Dead` (sole full-body pitch/roll exception), snapshot-authoritative return-or-capture travel; server capture rule when the defender is eliminated. N7g stays incomplete until Niclas passes the local manual/two-client combat gate ([VALIDATION_CHECKLIST.md](VALIDATION_CHECKLIST.md)) |
| **N8a** | **Done (2026-08; manual/visual gate PASS 2026-08-08)** | Minimal world cities: world `found_city` (settler consumed), **additive** snapshot `cities` + `next_city_id`, city rendering + selection at anchors — no territory, growth, population, or buildings (below) |
| **N8b** | **Done (2026-08; manual/visual gate PASS 2026-08-08)** | Flat yields v2 + production selection: world `set_city_production` (`produce_unit:warrior` / `produce_unit:settler` / `none`, **no unlock gating — locked**), **additive** city `current_project`, **flat per-city production yield** constant established (not ticked until N8c) on the existing canonical schema v1 — reads no tile properties, no content-schema expansion (below) |
| **N8R** | **Done (2026-08-08; architectural review PASS at `709810c`)** | Single-gameplay-core recovery: the PROVEN founding, production-selection, and production tick/delivery algorithms recovered into canonical map-neutral pure modules (`city_founding_rules.py`, `city_production_rules.py`) with map facts and the unlock gate as explicit adapter/policy inputs; `world_actions.py`/`world_legal_actions.py` AND the deprecated `Scenario` modules (`actions/found_city.py`, `actions/set_city_production.py`, `production_rules.py`) are fact adapters over that one core — no second validator/apply/tick/delivery algorithm anywhere; Godot N8 city UI extracted into focused components; behavioral-parity + dependency guard tests (below). **N8c is unblocked** |
| **N8c** | **Done (2026-08; manual/two-client runtime gate PENDING)** | Authoritative production processing + deterministic spawn (**requires N7g**, locked legacy-equivalent timing): world `end_turn` ticks the ending player's production, advances the turn, then **delivers completed units for the player who has just become current** (**additive** `next_unit_id`; producer city tile if unit-unoccupied, else first unit-unoccupied smooth-adjacent tile in `DIRECTIONS` order, else deferred with retained progress); event order `production_progress`* → `end_turn` → `unit_produced`* (below) |
...
**Status: implemented 2026-08-06 — manual/visual gate PENDING (not Done).** Clip audit passed before implementation: semantic `Walking`/`Idle_3` resolve through the audited remap to real shipped clips (settler `Running`/`Hit_Reaction_1`, warrior `Idle_02`/`Combat_Stance`; no clip carries non-skeleton root-motion position tracks). Rig audit (2026-08): both shipped rigs are AUTHORED facing +Z (glTF convention; toe-vs-ankle rest direction) — the character instance is mounted with one shared convention-level 180° yaw so the locked EFFECTIVE forward is local −Z; both rigs share the humanoid bone set `Hips` / `Left|RightUpLeg` / `Left|RightLeg` / `Left|RightFoot`. Locomotion layer in `world_units_view.gd` (root snaps to the anchor immediately, visual `ModelRoot` glides at `LOCOMOTION_SPEED_UNITS_PER_SEC = 1.6` world units/s ≈ 1.1 s per tile, yaw-only upright facing), `WorldSurfaceSampler` (top-surface-only raycast sampling, anchor-lerp fallback), `WorldUnitLegGrounder` (SkeletonModifier3D skeletal foot grounding, **uphill-grounding correction 2026-08-07**: joint two-leg pelvis solve via reach-feasibility intervals; analytic two-bone knee with an explicit per-leg anatomical pole from the audited rest toe-forward direction — never the current possibly-straight knee — with extension margins; absolute whole-foot sole alignment — the FINAL transformed rest-derived sole normal equals each foot's own effective contact normal exactly (sampled normal, anatomically clamped vs. the foot's animated heading; exact for in-clamp normals incl. mixed XZ; clip-authored tilt absorbed, heading preserved) with swing/contact blending; pose-derived bounded uphill-only swing clearance, zero at takeoff/landing; local-pose writes only; **third-review follow-up 2026-08-07 (sole-plane correction 2026-08-07):** sole-contact calibration — in contact the ankle target blends to the post-alignment sole-plane contact height: dot(n, ankle − s) = d, where d is the rig-derived signed ankle-to-sole-plane distance (= the rest ankle height, because both audited bind-pose soles sit exactly on the plane) and s the sampled plane point at the ankle's XZ — i.e. terrain + d / n.y, never the vertical terrain + d projection (only d·n.y of perpendicular clearance — the aligned sole would penetrate every slope); this removes the hover the remapped clips bake in by holding feet above rest — plus stationary foot planting: each foot of a non-gliding unit is anchored in ground space (position + normal-aligned orientation, held at the same post-alignment contact height over its planted anchor's own sample) with idle pelvis motion passing through and the plant weight blending smoothly around glide begin/arrival), production wiring + one-PC-debug F12 review screenshot in `cloud_world_play.gd`, and the fast `slice n7f` (309 checks incl. both-rig real-clip flat/shallow/medium/steep/downhill sequences and the planting/calibration regressions). Niclas's movement-read approval (pace, grounding, slope behavior) is outstanding — see the N7f section of [VALIDATION_CHECKLIST.md](VALIDATION_CHECKLIST.md).
...
- **Status:** sub-sliced 2026-08-07. **N7g.1 — server authority (implemented 2026-08-07):** authoritative state, `attack_unit` validation/resolution/persistence/turn gating exactly per the locked contracts below (literal reasons finalized in [CLOUD_API_V0.md](CLOUD_API_V0.md); resolution extracted into the shared pure `combat_rules.resolve_combat` core consumed by BOTH the legacy Scenario path and `world_actions` — one formula, no drift, no world-to-Scenario adapter). **N7g.2 — served attack rows (implemented 2026-08-07):** the legal-actions bullet below, derived through the SAME N7g.1 validators (`world_legal_actions._attack_actions_for_unit` — candidates in canonical `DIRECTIONS` order of the defender tile filtered through `validate_attack_unit_pre_map` + `validate_attack_unit_target`; never a second attack-legality implementation); selected-unit ordering is all attacks first, then all moves; summaries count both. **N7g.3 — client combat work (implemented 2026-08-07):** the client-work bullet below. N7g overall stays incomplete until Niclas passes the local manual/two-client combat gate ([VALIDATION_CHECKLIST.md](VALIDATION_CHECKLIST.md)).
...
- **Legal-actions (N7g.2, implemented 2026-08-07):** a selected own **warrior** additionally returns submit-ready `attack_unit` rows (adjacent enemy warriors over traversable edges), in canonical `DIRECTIONS` order of the defender tile, BEFORE the move rows; summaries count them (attacks + moves == the selected-unit row count; an attacked unit counts 0).
- **Tile occupation (locked N7g.3 correction, server-authoritative):** surviving defender → attacker stays on its original tile; defender eliminated and attacker survives → attacker **captures** the defender's former tile in the resulting snapshot; attacker killed by retaliation → no capture. Presentation reads the final destination from the deferred response snapshot only — never invents occupation from `defender_killed` alone.
- **Client work (N7g.3, implemented 2026-08-07; ninth visual-correction pass — manual gate PENDING):** attack-target markers from served rows (visually distinct from move markers in color and shape), submission of exact served rows, HP + `has_attacked` shown minimally (selection/status line), and **character combat presentation**: presentation-only melee approach (`Walking` to `MELEE_STANDOFF_DISTANCE` 0.80 at the stride-synchronized `LOCOMOTION_TRANSLATION_SPEED_SCALE` 0.445 with unchanged Walking animation playback 1.0; never emits `unit_arrived`), impact-timed overlapping attack with authored non-fatal `Hit_Reaction_1` or fatal `Dead` at `COMBAT_IMPACT_DELAY_SEC` ≈ 0.5 s (identical authored hit in both role directions; counterattack victim clears slash/aim and waits on Idle), combat-support grounding plus a swing-unit bounded Spine02+Spine01 chain solve of the forward-kinematic club-tip endpoint toward the defender's cached `Head` contact (per-bone `ATTACK_PITCH_MAX_RAD` 0.40, chain 0.80; `Hit_Reaction_1` not retargeted; equal elevation keeps the neutral authored slash; the killing swing rides its aim out through the victim's `Dead` — matched lethal/non-lethal strikes are trajectory-equivalent through contact), locomotion grounding/yaw/`0.445` through arrival Walking→Idle crossfade, every semantic clip change crossfades via `ANIM_BLEND_DEFAULT_SEC` 0.28, ordinary facing uses centralized `FACING_BLEND_SEC` 0.28 (return home restores ~180° toward the defender; occupation keeps travel facing), retaliation only when `retaliated == true`, **continuous corpse terrain support while `Dead` plays** (sole full-body pitch/roll exception), and survivor traversal to the **snapshot-authoritative** final anchor. Gameplay input + served legality stay blocked through the sequence until deferred snapshot apply + legality refetch. Outcomes come only from the accepted server event and snapshot; `CombatClashBurstView` remains excluded.
- **Verification:** server tests — full rejection order, math parity against the legacy `combat_rules` constants, retaliation/elimination/flag-reset invariants, drift-500 read-only (N7g.1: `server/tests/test_world_combat_v3.py`, in server `slice n7`), legal-actions round trip (every advertised attack accepted on an equivalent match — N7g.2, landed in `server/tests/test_world_legal_actions_v3.py`); Godot tests — distinct attack markers, exact served-row submission, stale-row rejection, HP/`has_attacked` propagation, combat sequencing/facing/eliminations/cancellation, and gate/restoration wiring (N7g.3, landed in Godot `slice n7g3`); the local manual/two-client combat gate in [VALIDATION_CHECKLIST.md](VALIDATION_CHECKLIST.md) (PENDING).
...
- **Status: Done (2026-08) — manual/visual gate PASS (2026-08-08).** Niclas approved: Found City works; the Settler disappears; the city is visible and understandable; unit/city interaction looks correct. Absence of the old deprecated Scenario/HexMap gameplay menus is expected and approved — that obsolete UI must remain disconnected and must not be restored.
- **Objective:** settlers found cities; cities are visible and selectable — target-loop points 4–5. Prereqs: N7d.
- **Authoritative state (additive snapshot v3):** **`cities`** (deterministic rows sorted by `id`: `{"id", "owner_id", "position": [q, r], "name"}`) and **`next_city_id`** (initial `1`); names reuse the canonical rule (`Capital`, then `Settlement 2`, … per owner). World **`found_city`** (legacy wire shape: `schema_version` 1, `actor_id`, `unit_id`, `position`) consumes the founding settler atomically with city creation. Locked first-failure order: `wrong_action_type → unsupported_schema_version → malformed_action → not_current_player → unknown_unit → unit_not_owned_by_player → unit_cannot_found_city → unit_not_at_position → tile_already_has_city` (no water/territory reasons — schema v1 has neither; founding legality never reads elevation). **Locked (2026-08-06):** cities do **not** block unit movement (`destination_occupied` stays units-only); N8 has **no minimum city distance** — after the standard action/unit/map validation, an existing city on the founding tile is the **only** city-placement restriction.
- **Events / turn processing:** accepted founding bumps revision, persists, appends a legacy-shaped `found_city` event (`city_id`, `city_name`, `at`, `settler_consumed`); no ticks on founding. `set_city_production` remains `unsupported_action_for_match_kind` until N8b.
- **Legal-actions:** a selected own settler returns attacks (none), then the submit-ready `found_city` row when its tile has no city, then moves; unit summary counts include found_city; `city_summaries` lists the actor's cities with `legal_action_count` 0 (empty actions for city selection until N8b).
- **Client work (shipped):** `WorldCitiesView` (`game/presentation/world/world_cities_view.gd`) — one stable root per city id at the N4 tile anchor, reusing `ancient_village` in place; “Found City” button when a served row is fresh; city selection via tile pick with the **locked shared-tile rule**; minimal selected-city status line. Client never optimistically creates a city or consumes the Settler — only the authoritative snapshot does. Consumed selected settlers clear selection safely on snapshot apply.
- **Verification:** server `slice n8a` (`test_world_found_city_v3.py` + legal-actions/match shape); Godot `slice n8a` (`test_world_cities_view.gd`, `test_world_found_city_interaction.gd`, `test_world_interaction_state.gd`).
- **Manual/visual gate: PASS (2026-08-08).** City marker placement/readability and shared-tile selection feedback approved by Niclas.
- **N8a / N8b boundary:** N8a ends when the city exists, the Settler is consumed, and both clients can display and select the authoritative result. N8b owns flat yields + `set_city_production` / `current_project` — not founding.
- **Non-goals:** territory/owned tiles, population/growth/food, buildings, borders, city panels beyond the status line, city combat, production, science, AI settlement.
- **Done / merge boundary:** two clients can found and see cities; one PR.

#### N8b — Flat yields v2 + production selection

- **Status:** Done (2026-08) — **manual/visual gate PASS (2026-08-08).** Niclas approved: production selection and progress/cost presentation look correct.
...
- **Manual/visual gate: PASS (2026-08-08).** Production-choice UI readability and progress/cost presentation approved by Niclas.
...
#### N8R — Single-gameplay-core architecture recovery

- **Status:** Done (2026-08-08) — **architectural review PASS at reviewed commit `709810c613d63d93417a1e35093a8212665ef239`.** The complete map-independent founding transition and the production selection/tick/delivery core are accepted as the one canonical implementation. N8a/N8b manual/visual gates also PASS the same day; the stack is authorized to merge into `main`.
- **Objective:** correct the drift toward a second, simplified WorldMap rules engine by recovering the PROVEN game loop as the canonical implementation. ONE canonical map-neutral core — `server/app/domain/city_founding_rules.py` (default Capital/Settlement-N naming, the ONE founding decision chain with the map-dependent tile facts — on-map/water/existing-city/territory — as explicit adapter inputs whose defaults describe the layer-less WorldMap schema, and the COMPLETE map-independent founding transition — `found_city_transition` returns one typed immutable value carrying the consumed founder unit id, allocated city id, advanced `next_city_id`, owner, position, canonical name, first-city capital semantic, and empty initial project, so neither state adapter independently decides founder consumption or city-id allocation/advancement; the legacy adapter's `initial_owned_tiles_for_city` stays a FROZEN legacy `HexMap` territory policy — deprecated-path gameplay, never shared WorldMap gameplay) and `server/app/domain/city_production_rules.py` (the ONE production-selection decision chain with the legacy progress-unlock gate as an explicit policy input, canonical `{project_id, progress, cost}` project state with DERIVED completion — the snapshot-v2 `ready` flag is that predicate materialized by its adapter, the PROVEN production tick + delivery loop moved out of the Scenario-coupled legacy engine — eligibility, ascending city-id ordering, sequential unit-id allocation, project clearing, `production_progress`/`unit_produced` engine-event construction, with yields and spawn positions as caller-supplied facts, the event progress representation, `FLAT_PRODUCTION_PER_CITY` — the former `world_production_rules.py` is removed). `world_actions.py` / `world_legal_actions.py` are thin snapshot-v3/WorldMap adapters (wire envelope checks, snapshot fact extraction, state materialization, map facts) over that core; the deprecated `Scenario` modules (`actions/found_city.py`, `actions/set_city_production.py`, `production_rules.py`) are Scenario fact adapters over the SAME core (envelope parsing, Scenario fact extraction, snapshot-v2 materialization incl. territory/palace/`project_type`/`ready` storage keys) — no second founding or production validator, apply, tick, or delivery algorithm exists anywhere, and the API's legacy reason remaps were deleted as identity maps (the legacy validators return the canonical wire strings directly).
- **Client:** the N8 city UI is extracted from `cloud_world_play.gd` into focused components — `world_city_selection_ui.gd` (Found City button) and `world_city_production_panel.gd` (production panel) — reporting choices via signals; the scene stays composition/wiring and still submits only exact served rows.
- **Guards:** `server/tests/test_city_rules_canonical.py` (pure-rule behavior with plain facts; BEHAVIORAL PARITY — equivalent founding/production facts through the deprecated Scenario adapter and the active snapshot-v3 adapter produce identical decisions and semantically identical transitions; pure-fact tick/delivery coverage incl. N8c flat-yield readiness with no Scenario; limited single-implementation patch guards — patching the one canonical validator OR the one canonical founding transition changes BOTH state adapters' decisions and apply results, and the world legal-actions derivation; AST dependency walls — canonical rules import no map/transport/persistence, world modules import no deprecated snapshot-v2 gameplay, the Scenario modules must import the canonical rules) and `game/cloud/tests/test_world_city_ui_components.gd` (components render served rows only; composition + no-local-production-loop guards). External N8a/N8b behavior byte-identical on both paths (schemas, literal wire rejection reasons, ordering, snapshots, events, hashes).
- **Locked principles:** see [ARCHITECTURE_PRINCIPLES.md](ARCHITECTURE_PRINCIPLES.md) "One canonical gameplay core" — adapters may differ, gameplay rules may not; extract, never copy; no match-kind-specific copies; the deprecated runtime path is not a requirement. **N8c** was unblocked by the N8R review PASS and has since shipped as its own slice (not implemented by N8R).

#### N8c — Authoritative production processing + deterministic spawn

- **Status:** Done (2026-08) — **manual/two-client runtime gate PENDING** (Niclas). Deterministic server coverage green (`slice n8c`).
- **Objective:** production advances through authoritative turn processing and completed units spawn deterministically into the continuing loop — target-loop points 7–8. Prereqs: **N8b and N7g** (produced units carry the N7g `current_hp` and `has_attacked` fields); unblocked by N8R architectural review PASS at `709810c` (2026-08-08).
- **Implementation (shipped):** reuses the ONE canonical loop in `city_production_rules.tick_production` / `deliver_completed_production` (N8R). WorldMap `end_turn` is a thin adapter: flat yields (`FLAT_PRODUCTION_PER_CITY`), WorldMap spawn placement via an optional delivery resolver (city tile if unit-unoccupied, else first unit-unoccupied smooth-adjacent tile in `DIRECTIONS` order; `None` defers without allocating an id / clearing the project / emitting `unit_produced`), and snapshot-v3 materialization (`next_unit_id`, unit rows with full `current_hp` + `has_attacked: false`). Never a second tick/delivery algorithm.
- **Turn processing (locked 2026-08-06 — legacy-equivalent timing):** on an accepted world `end_turn` by player P, in this order inside one apply: **(1) production tick** — for each of P's cities in ascending city-id order with a non-null, **not yet completed** project: `progress += flat production yield` (**`progress >= cost` means completed/ready; completed projects no longer accrue production**); **(2) turn advance** — `turn_state` advances and every `has_attacked` flag clears (N7g rule); **(3) delivery for the player who has just become current** — for each of that player's **ready** cities in ascending city-id order, attempt deterministic placement with **occupancy updated after every successful spawn**; only after successful placement is `next_unit_id` allocated (then incremented; **additive** snapshot field), the unit spawned, `unit_produced` emitted, and `current_project` cleared to `null`; if no placement exists, the completed project **and its progress are retained unchanged** for retry the next time the owner becomes current. A produced unit becomes available exactly when its owner gains control and **cannot be attacked during the intervening opponent turn**.
- **Events (locked order):** `production_progress`* → `end_turn` → `unit_produced`*. The POST response's primary **`event`** and **`index`** remain the accepted **`end_turn`** row.
- **Legal-actions:** unchanged shapes; new units appear in summaries/moves.
- **Client work:** none beyond existing `WorldUnitsView` snapshot reconciliation (new unit ids at supplied anchors).
- **Verification:** server `slice n8c` (`test_world_production_v3.py` + canonical city-rule guards); Godot `test_world_units_view.gd` covers new-id placement at anchors; manual/two-client produce→spawn→act round PENDING for Niclas.
...
- **Done / merge boundary:** deterministic produce→spawn→act cycle covered server-side; one PR; manual gate remains open.
