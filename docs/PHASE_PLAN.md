...
| **N3c+** | **Closed (2026-08)** | N3 terrain construction complete with N3c.7. Final sky art direction, clouds, and richer atmosphere: a separate future slice. Next planned slice: **N5** (server `WorldMap` foundation) |
...
| **N5** | Planned | Server logical `WorldMap` foundation + canonical content packaging: Python `WorldMap` domain mirror plus a server-owned canonical map-content loader under `server/app/` (raw-byte SHA-256 `content_hash`, schema v1 validation, locked `edge_rule` derivation; never imports `tools/blender/`), cross-language identity/edge parity goldens vs the Godot loader, and explicit server content packaging (stable canonical content path in local and container execution; canonical content shipped in the server image; byte-identical/LF-safe distribution with hash stability; verified from a server-like or built-container environment). Server tests cover valid identity, unknown `map_id`, load failures, and canonical hash generation. TS-08 solving and geometry stay Godot-only — the server holds tiles/elevations/edges, never geometry. No match kind, snapshot changes, or units |
| **N6** | Planned | Snapshot v3 + server-fed client world bootstrap (no gameplay): a `WorldMap` match kind beside the legacy path; snapshot v3 = `MapIdentity` (`map_id`, `schema_version`, `content_hash`) plus mutable match state, **no embedded tiles/edges**; the client loads local canonical content by `map_id` and verifies `content_hash` against the server identity — mismatch fails explicitly, never a silent fallback (verified primarily in the Godot bootstrap tests); production world bootstrap around `TerrainWorld` + the N4 anchor UI. Legacy v2 flow untouched; default entry unchanged until N9. Production-scene placement and match-kind routing are decided at N6 planning |
| **N7** | Planned | Server-authoritative units, movement, and actions on the `WorldMap` match kind: server owns unit state, legality, and apply (movement v1 = adjacency + occupancy + the locked edge rule — cliff edges block; elevation/cliff/occupancy semantics only, no water/terrain-category passability without a separately justified schema change); client renders units at the N4 `tile_anchors` and submits actions through the same client-server API path. No client-side legality, ever |
| **N8** | Planned | Yields v2 + minimal cities on the `WorldMap` match core using **flat base yields** on the existing canonical schema v1 — no content-schema expansion pre-approved (water, terrain categories, and passability require a separately justified schema change in N8 or later) |
