...
| **N8R** | **Implemented (2026-08-08; architectural review PENDING)** | Single-gameplay-core recovery: N8a/N8b rules consolidated into canonical map-neutral pure modules (`city_founding_rules.py`, `city_production_rules.py`); `world_actions.py`/`world_legal_actions.py` reduced to thin adapters; deprecated-path duplicates delegate to the core; Godot N8 city UI extracted into focused components; architecture guard tests added (below). **N8c is blocked until N8R has been reviewed** |
...
- **Status:** implemented 2026-08-08; **architectural review PENDING** (stacked on pending N8a/N8b; no new manual/visual behavior — the N8a/N8b gates carry over unchanged).
- **Objective:** correct the drift toward a second, simplified WorldMap rules engine. The N8a/N8b gameplay rules now live in ONE canonical map-neutral core — `server/app/domain/city_founding_rules.py` (default Capital/Settlement-N naming, locked founding decision chain + rejection reasons, founding effect) and `server/app/domain/city_production_rules.py` (locked production-selection decision chain + rejection reasons, canonical `{project_id, progress, cost}` project state, event progress representation, `FLAT_PRODUCTION_PER_CITY` — the former `world_production_rules.py` is removed). `world_actions.py` / `world_legal_actions.py` are thin snapshot-v3/WorldMap adapters (wire envelope checks, snapshot fact extraction, state materialization, map facts) over that core; the deprecated `Scenario` action modules delegate their shared rules (naming, event progress) to the same core instead of keeping copies.
...
- **Guards:** `server/tests/test_city_rules_canonical.py` (pure-rule behavior with plain facts; patched-canonical delegation proof for POST validators, apply, and legal-actions derivation; AST dependency walls — canonical rules import no map/transport/persistence, world modules import no deprecated snapshot-v2 gameplay) and `game/cloud/tests/test_world_city_ui_components.gd` (components render served rows only; composition + no-local-production-loop guards). External N8a/N8b behavior byte-identical (schemas, literal rejection reasons, ordering, snapshots, events, hashes).
