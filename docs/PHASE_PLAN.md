...
| **N7d** | Planned | Client world interaction loop (client-only): unit selection from terrain picks, **server-fed** legal destinations/actions rendered as markers, `move_unit` submission of exact served rows, End Turn, out-of-turn snapshot polling, minimal turn/status UI — no combat, cities, or movement animation (see **Planned N7d–N8d** below) |
| **N7e** | Planned | N7 verification checkpoint: deterministic suites + documented manual two-client validation of the movement/turn loop (incl. remote authority); `VALIDATION_CHECKLIST.md` entry — no new gameplay (below) |
| **N7f** | Planned | Unit locomotion presentation: walking animation between anchors, movement facing, slope alignment, foot grounding — presentation-only polish; never a legality/authority change; gates nothing in N7g/N8 (below) |
| **N7g** | Planned | World Combat 0.1: server-authoritative `attack_unit` on world matches with exact Local Combat 0.1 math parity; **additive** snapshot v3 unit `current_hp` + per-turn attacked flag; served attack rows; minimal client feedback — **proposed contracts pending review** (below) |
...
| **N8b** | Planned | Flat yields v2 + production selection: world `set_city_production` (`produce_unit:warrior` / `produce_unit:settler` / `none`, **no unlock gating** — proposed), **additive** city `current_project`, **flat per-city production yield** on the existing canonical schema v1 — reads no tile properties, no content-schema expansion (below) |
| **N8c** | Planned | Authoritative production processing + deterministic spawn: world `end_turn` accrues production for the ending player's cities and spawns completed units deterministically (**additive** `next_unit_id`; city tile if free, else first unoccupied smooth-adjacent tile in `DIRECTIONS` order, else deferred); legacy-named engine events (below) |
...
**Sequencing.** Recommended order: **N7d → N7e → N7f → N7g → N8a → N8b → N8c → N8d**. Hard dependencies: N7d ← N7a–c; N7e ← N7d; N7g ← N7d; N8a ← N7d; N8b ← N8a; N8c ← N8b; N8d ← N7d/N7e/N7g/N8a–N8c. **N7f depends only on N7d and gates nothing** — it may land any time before N8d without blocking N7g/N8. N7g and N8a are mutually independent (either order works); the recommended order keeps combat inside the N7 unit-action family.

**Standing constraints (every slice below).** Server authority and **served legality only** — clients never compute world legality; deterministic snapshots, `state_hash`, and events through the existing envelope (`canonical_json`); canonical content **schema v1 untouched** — no water, terrain-category, or passability fields (any such change needs a separately justified schema slice); snapshot v3 evolves **additively only**; flat initial yields and deliberately minimal city/production rules; no new terrain, camera, or asset work (existing GLBs/markers/UI patterns reused in place); no new compatibility investment in the deprecated legacy path (`main.tscn`, snapshot v2, `HexMap`); **N9 remains full cutover + legacy removal** and is not pulled forward. World-match creation stays env-opt-in (`EOM_CLOUD_MATCH_KIND=world_map`) through N8 — front-door world-creation UI is **not** part of this plan.
...
- **Client work (`cloud_world_play` + presentation/world):** unit selection driven by the locked N4 pick contract (`terrain_picked`): a tile pick on a tile holding an own unit selects that unit (presentation state only); a miss clears; a cliff pick leaves selection unchanged. On selection, `GET .../legal-actions?selected_unit_id=…`; the served `move_unit` rows are rendered as destination markers at the corresponding `tile_anchors` — the marker set is exactly the served rows, never client-derived. Clicking a marked destination POSTs **that exact served action row**; the accepted response snapshot replaces the held snapshot and units re-render at their new anchors (instant reposition — movement animation is N7f). End Turn control POSTs the summary-mode submit-ready `end_turn`. Minimal turn/status UI: current player + own seat identity from `turn_state`/staging identity (reusing the existing display-name helpers), plus rejection/error surfacing. Out-of-turn: action input disabled; conservative revision-based snapshot polling of `GET /v1/matches/{id}` (reusing the shipped poll-gating pattern from the legacy cloud waiting flow — no websockets, no event replay), applying newer revisions to the unit view.
- **Verification:** Godot headless tests — pick→selection mapping (incl. cliff-pick unchanged + miss-clear), served-row→marker mapping with no client legality, exact submitted payloads, poll gating states, End Turn flow; server slice `n7` suites stay green untouched. Two-client check via two instances (`EOM_CLOUD_PROFILE`) against one local server: alternate move/End Turn, opponent movement appears via polling.
- **Manual/visual gate:** short two-instance session reviewed by Niclas (selection feel, marker readability).
...
- **Done / merge boundary:** two clients can alternate authoritative turns moving units with all legality served; client-only PR; legacy path untouched.

#### N7e — N7 verification checkpoint
...
- **Verification:** server `slice n7` + Godot `slice n7` + `smoke` green; deliberate negative checks exercised once — content-drift HTTP 500 via `EMPIRE_MAP_CONTENT_DIR` misdirection (read-only, no mutation), out-of-turn and cliff-blocked rejections through the real client path; manual two-client session against the **remote** authority (Hetzner) including reconnect mid-match; post-build image check (`tools/content/check_server_image_map_content.py`) when the deploy is refreshed.
- **Non-goals:** new gameplay, new endpoints, performance work.
- **Done / merge boundary:** checklist section recorded with results; N7a–N7e marked done here; docs-only (plus test wiring) PR.

#### N7f — Unit locomotion presentation

- **Objective:** replace instant repositioning with readable movement: walking animation between anchors, facing along the movement direction, slope alignment, and foot grounding on the solved surface. Prereqs: N7d (gates nothing afterwards).
- **Authoritative changes:** none. Anchors remain the placement authority; interpolation between the old and new anchor is transient presentation state; grounding/slope may sample the **rendered** surface/collision for visuals only — never for legality (N3c.4 boundary). The authoritative snapshot is applied immediately; only the visual catches up.
- **Client work:** `world_units_view.gd` tween/animation layer using the existing audited GLB clips (walk/idle); deterministic facing rule (locked −Z forward toward the destination anchor, azimuth-only unless slope alignment says otherwise); no new assets — if a required clip does not exist in the shipped GLBs, the slice stops and reports instead of importing assets.
...
- **Authoritative state (additive snapshot v3):** unit rows gain **`current_hp`** (int; full = server unit-definitions `max_hp`, today 100 — max stays registry data, not snapshot state) and **`has_attacked`** (bool; reset for all units on every accepted `end_turn`). Existing persisted pre-N7g world matches are **not migrated** (dev/alpha store; recreate matches) — *proposed*.
- **Action / legality:** world **`attack_unit`** with the exact legacy wire shape (`schema_version` 1, `actor_id`, `attacker_id`, `defender_id`). Resolution reuses the **canonical Local Combat 0.1 math verbatim** (`BASE_DAMAGE` 30, `exp((atk−def)/25)` scaling, clamp 1–100, retaliation only if the defender survives, elimination at 0 HP, attacker never advances into the tile) — warrior attacker **and** warrior defender, matching the canonical contract. World-specific rules (*proposed*): melee requires a **traversable (smooth) edge** — a cliff or missing edge record blocks the attack fail-closed (single edge-legality source shared with movement); after attacking, the unit can neither move nor attack again until its owner's next turn (`has_attacked` checked in move validation — mirrors the legacy “attack sets `remaining_movement` to 0” without introducing movement points); move-then-attack stays allowed. Proposed first-failure order (literal reasons finalized with the implementation in [CLOUD_API_V0.md](CLOUD_API_V0.md)): `wrong_action_type → unsupported_schema_version → malformed_action → not_current_player → unknown_attacker → unknown_defender → actor_not_owner → attacker_not_warrior → defender_not_warrior → cannot_attack_own_unit → attacker_already_attacked → defender_not_adjacent → attack_edge_missing → attack_cliff_blocked`; map identity resolved fail-closed before destination-side checks, as N7a.
...
- **Legal-actions:** a selected own **warrior** additionally returns submit-ready `attack_unit` rows (adjacent enemy warriors over traversable edges), in canonical `DIRECTIONS` order of the defender tile; summaries count them.
- **Client work:** attack-target markers from served rows (distinct from move markers), submission of exact served rows, HP shown minimally (selection/status line), eliminated units disappear on snapshot apply. No clash animation (the legacy `CombatClashBurstView` is not reused).
- **Verification:** server tests — full rejection order, math parity against the legacy `combat_rules` constants, retaliation/elimination/flag-reset invariants, drift-500 read-only, legal-actions round trip (every advertised attack accepted on an equivalent match); Godot tests — marker mapping, payloads, HP/elimination rendering; two-client manual combat session.
- **Manual/visual gate:** short two-client combat exchange approved by Niclas.
- **Non-goals:** attacks on non-warrior defenders (flagged open decision — see proposed-contract list), ranged/city combat, terrain/elevation combat modifiers, movement points, zone of control, combat animation, AI.
...
- **Authoritative state (additive snapshot v3):** **`cities`** (deterministic rows sorted by `id`: `{"id", "owner_id", "position": [q, r], "name"}`) and **`next_city_id`**; names reuse the canonical rule (`Capital`, then `Settlement 2`, … per owner). World **`found_city`** (legacy wire shape: `schema_version` 1, `actor_id`, `unit_id`, `position`) consumes the founding settler. *Proposed* first-failure order: `wrong_action_type → unsupported_schema_version → malformed_action → not_current_player → unknown_unit → unit_not_owned_by_player → unit_cannot_found_city → unit_not_at_position → tile_already_has_city` (no water/territory reasons — schema v1 has neither; founding legality never reads elevation). *Proposed:* cities do **not** block unit movement (`destination_occupied` stays units-only); no minimum city distance in N8.
...
- **Client work:** `WorldCitiesView` following the `world_units_view.gd` pattern — one stable root per city id at the tile anchor, reusing an existing city marker/asset in place (no new assets); “Found City” affordance when a served row exists; city selection via tile pick (*proposed* shared-tile precedence: first pick selects the unit, picking the same tile again cycles to the city); minimal selected-city status line.
...
- **Manual/visual gate:** city marker placement/readability approved.
...
- **Yields v2 (initial instantiation, *proposed*):** every city yields a **flat base production of 1 per turn** — a constant read from server-owned rules data, **reading no tile properties** (works on canonical schema v1 exactly as-is; MAP_MODEL.md layer 9 “flat base yields”). No food, science, or coin on the world path in N8; per-tile/worked-tile yields, population, and richer yield content stay N9+ evaluation.
- **Authoritative state (additive):** city rows gain **`current_project`** (`null` or `{"project_id", "progress", "cost"}`); costs come from the existing server `CityProjectDefinitions` registry (`produce_unit:warrior` 2, `produce_unit:settler` 2 — balance placeholders). World **`set_city_production`** (legacy wire parity: `schema_version` **2**, `project_id` ∈ `produce_unit:warrior` | `produce_unit:settler` | `none`). *Proposed:* **no unlock gating** — there is no world `progress_state`/science in N8, so both projects are always selectable (`city_project_not_unlocked` does not exist on the world path). *Proposed* first-failure order: `wrong_action_type → unsupported_schema_version → malformed_action → not_current_player → unknown_city → city_not_owned_by_player → unknown_city_project → project_already_set`.
...
- **Objective:** production advances through authoritative turn processing and completed units spawn deterministically into the continuing loop — target-loop points 7–8. Prereqs: N8b.
- **Turn processing (*proposed* order, locked at implementation):** on an accepted world `end_turn` by player P, before the turn advances: for each of P's cities in ascending city-id order with a non-null project — `progress += flat production yield`; if `progress >= cost`, spawn a completed unit: **spawn placement rule (*proposed*):** the city tile if unoccupied, else the first unoccupied tile adjacent over a **smooth** edge in canonical `DIRECTIONS` order; if none exists, the spawn is **deferred** (progress is retained and the attempt repeats on the next processing — no unit is lost, nothing random). Spawned units take `id = next_unit_id` (then increment; **additive** snapshot field introduced here), full `current_hp`, `has_attacked` false; the completed project resets to `null` (repeat production is deferred). Then `turn_state` advances and `has_attacked` flags clear (N7g rule). *Proposed divergence from legacy:* the legacy path delivers units when the owner **becomes** current (movement-refresh timing); the world path has no movement points, so spawning inside the owner's own `end_turn` apply is equivalent for play and keeps one mutation point — flagged for review.
- **Events:** the accepted `end_turn` row stays the action event; engine follow-ups append with the **legacy event names** — `production_progress` per accruing city and `unit_produced` per spawn (fields mirroring the legacy rows where semantics match).
...
- **Verification:** server tests — accrual/cost/spawn determinism incl. occupied-city-tile fallback order, deferred-spawn retention, id sequencing, event rows, drift-500; Godot test — produced unit appears at the correct anchor; two-client production round.
...
#### Proposed contracts pending review (Niclas)

The plan deliberately does **not** silently canonize new gameplay rules. The following are **proposals with rationale**, to be approved (or amended) before their slices are implemented; everything else above reuses existing canonical contracts (Local Combat 0.1 math, legacy wire shapes, project registry, city naming, event names).

1. **N7g attack gating without movement points:** additive per-unit `has_attacked` flag; a unit that attacked can neither move nor attack again until its owner's next turn (move-then-attack allowed). Mirrors the legacy “attack sets MP to 0” using minimal deterministic state instead of importing a movement-point system.
2. **N7g melee edge rule:** attacks require a traversable (smooth) edge; cliff or missing edge record blocks fail-closed. Keeps one edge-legality source for movement and melee; prevents un-reachable-tile melee.
3. **N7g defender restriction:** keep the canonical warrior-vs-warrior restriction. **Open decision:** whether warriors may attack settlers (would let combat threaten expansion; deviates from Local Combat 0.1) — explicitly *not* included until decided.
4. **N7g store compatibility:** persisted pre-N7g world matches are not migrated (dev/alpha store; recreate matches).
5. **N8a city passability & spacing:** cities never block unit movement; no minimum distance between cities; founding rejects only a tile that already has a city.
6. **N8a shared-tile selection precedence:** first pick selects the unit, repeated pick on the same tile cycles to the city (presentation-only).
7. **N8b no unlock gating / no world science in N8:** both `produce_unit` projects always selectable; the world path carries no `progress_state`. Science/research on the world path is a post-N8 decision.
8. **N8b flat yield value:** flat base production **1 per city per turn** (with registry costs 2/2 → a unit every second turn). Balance placeholder, trivially retunable.
9. **N8c spawn rule & timing:** spawn on the owner's own `end_turn` (before turn advance) at the city tile, else first unoccupied smooth-adjacent tile in `DIRECTIONS` order, else deferred with retained progress; completed project resets to `null`. Diverges from legacy delayed delivery (rationale above).
10. **N8c id counters:** introduce additive snapshot `next_unit_id` at N8c (and `next_city_id` at N8a) rather than deriving ids from max-existing (stable under elimination).
