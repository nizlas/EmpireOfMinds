...
- **Manual/visual gate (locked N7d gate shape):** **one local authoritative FastAPI process + ONE Godot instance** in the one-PC debug mode (`EOM_CLOUD_ONE_PC_DEBUG=1`), controlling both players in turn per the locked dual-entry direction — reviewed by Niclas (selection feel, marker readability, End Turn handoff between the two controlled players). Genuine two-client verification (two `EOM_CLOUD_PROFILE` instances, local and remote authority) belongs to **N7e**, not this gate.
...
- **Verification:** server `slice n7` + Godot `slice n7` + `smoke` green; deliberate negative checks exercised once — content-drift HTTP 500 via `EMPIRE_MAP_CONTENT_DIR` misdirection (read-only, no mutation), out-of-turn and cliff-blocked rejections through the real client path; **genuine two-client verification** — two Godot instances (`EOM_CLOUD_PROFILE`) against one local server AND a manual two-client session against the **remote** authority (Hetzner) including reconnect mid-match (N7e owns all two-client checks; the N7d gate is the one-PC debug mode); post-build image check (`tools/content/check_server_image_map_content.py`) when the deploy is refreshed.
