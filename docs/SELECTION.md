...
Scene order under **`Main`**: **`TurnLabel`** (HUD) → **`MapView`** → **`CitiesView`** → **`SelectionView`** → **`UnitsView`** → **`TerrainForegroundView`** → **`SelectionController`** → **`EndTurnController`** / **`AITurnController`** / **`LogView`** (see **[main.tscn](../game/main.tscn)**).
