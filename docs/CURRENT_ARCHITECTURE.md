...
| **HUD** | **`HudCanvas`** in **`main.tscn`** — **`TurnStatusPanel`** (**`turn_status_panel.gd`**, lower-right **current-player** strip for **local hotseat**: **`Player N's turn`**, colors from **`UnitNameplateView.owner_nameplate_accent_color`** like **`EmpireBorderView`** / nameplates; **remote “waiting for …”** semantics **deferred**) + **selected-city hub** (**`city_production_panel.gd`**, **`CityProductionPanel`**) + **`CityViewState`** (**`city_view_state.gd`**, **NORMAL**/**PLANNING** presentation submode); discovery/science panels, popups, yields toggle; **`main.gd`** wires hub **`Close`** redraw refs (**`selection_view`**, **`city_territory_view`**, **`city_worked_tiles_view`**). Map stack includes **always-on** **`EmpireBorderView`**. |
...
- **Runner:** from repo root, [scripts/run-godot-tests.ps1](../scripts/run-godot-tests.ps1) runs a **fixed list (~107)** of Godot headless scripts (**`-s res://…`**).
