...
|------|-------------------------|
| **`game/domain/world/` (N1, N3a, N3b)** | **`WorldMap`**, **`MapIdentity`**, **`HexWorldProjection`**, **`MapContentLoader`** (N1); **`Ts08TerrainMath`**, **`Ts08CutLattice`** (N3a Stage-0 cut-lattice topology from `WorldMap` only); **`Ts08HeightSolver`** (N3b Stage-2 cut-domain thin-plate CG height solve from `WorldMap` + the N3a lattice only) — not wired to gameplay or presentation yet |
