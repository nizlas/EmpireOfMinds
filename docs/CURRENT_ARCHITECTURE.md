...
- **TS-08 reference dataset (N2, 2026-08):** deterministic Stage-2 export at **`content/terrain/reference/handdrawn_test_map_full_01_ts08_stage2_reference_v1.json`** — engine-neutral parity authority for Godot construction (not the `.blend`). Exporter/audit: `tools/blender/terrain/export_ts08_reference_dataset.py`; schema and contract in **`content/terrain/reference/README.md`**.
- **Blender reference/tooling (development-only, outside the game):** the TS-08 chain under `tools/blender/terrain/` (see its `README.md` and `TERRAIN_PROTOTYPE_MANIFEST.md`). Never a runtime dependency; the Blender mesh is not the parity authority (the N2 dataset is).
