...
|------|-------------------------|
| **`game/domain/world/` (N1, N3a)** | **`WorldMap`**, **`MapIdentity`**, **`HexWorldProjection`**, **`MapContentLoader`** (N1); **`Ts08TerrainMath`**, **`Ts08CutLattice`** (N3a Stage-0 cut-lattice topology from `WorldMap` only) — not wired to gameplay or presentation yet |
...
- **Approved target (not implemented as gameplay):** **`WorldMap`** foundation implemented (slice **N1**, 2026-08) under `game/domain/world/` — loads packaged canonical map content, derives edges, and exposes projection math. **N3a (2026-08):** native TS-08 Stage-0 cut-lattice topology builder (`Ts08CutLattice`) constructs Ω_cut from **`WorldMap` only** (no height solve, no mesh). Parity locked via a test-only **digest manifest** derived from N2 — counts plus SHA-256 digests over complete canonical topology streams, no raw arrays (`game/domain/tests/fixtures/world/handdrawn_test_map_full_01_ts08_n3a_topology_parity_v1.json`; generator `tools/blender/terrain/generate_ts08_n3a_parity_manifest.py`). **Not yet connected** to gameplay, server, or 3D rendering. Full 3D world integration continues with N3b–N8 — see [PHASE_PLAN.md](PHASE_PLAN.md), [WORLD_COORDINATES.md](WORLD_COORDINATES.md), [MAP_MODEL.md](MAP_MODEL.md).
