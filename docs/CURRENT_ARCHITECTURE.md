...
- **Map content (approved, not yet implemented):** repo-root `content/maps/` with `reference/`, `authored/`, and `generated/` subdirectories ([MAP_CONTENT.md](MAP_CONTENT.md)). The TS-08 reference map will move to `content/maps/reference/handdrawn_test_map_full_01.json` (JSON envelope schema v1). **Current implementation:** the grid is still embedded as `TERRAIN_MAP_JSON` in `tools/blender/terrain/generate_terrain_terrainmap_handdrawn_full_01.py`; extraction is planned for Slice B+C.
