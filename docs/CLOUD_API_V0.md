...
**`units` (N7a):** deterministic, sorted ascending by **`id`**; each row is exactly `{"id", "owner_id", "position": [q, r], "type_id"}` with `type_id` ∈ `settler` | `warrior`. There is **no** `next_unit_id`, HP, movement points, or moved flag on the world path — world movement v1 has no movement budget ([MOVEMENT_RULES.md](MOVEMENT_RULES.md)). Planned **additive** v3 fields land with their slices ([PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”): unit `current_hp` + per-turn attacked flag (N7g), `cities` + `next_city_id` (N8a), city `current_project` (N8b), `next_unit_id` (N8c); wire contracts are specified here when each slice is implemented.
...
Only **`move_unit`** and **`end_turn`** exist on the world path (wire shapes identical to the legacy actions below, both `schema_version` `1`). Known legacy action types **`found_city`**, **`set_city_production`**, and **`attack_unit`** reject with **`unsupported_action_for_match_kind`** (their world equivalents are planned: `attack_unit` → N7g, `found_city` → N8a, `set_city_production` → N8b — [PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”; each keeps this rejection until its slice lands); unrecognized types reject with **`unknown_action_type`**. World action fields use **exact JSON integer** semantics: `schema_version` must be a non-boolean integer equal to `1` (boolean `true` and `1.0` reject as `unsupported_schema_version`); `actor_id` and `unit_id` must be non-boolean integers (`malformed_action` otherwise); coordinates were already exact-integer pairs.
