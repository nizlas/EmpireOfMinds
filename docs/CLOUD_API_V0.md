...
**`units` (N7a; combat fields N7g.1):** deterministic, sorted ascending by **`id`**; each row is exactly `{"id", "owner_id", "position": [q, r], "type_id", "current_hp", "has_attacked"}` with `type_id` ∈ `settler` | `warrior`. **`current_hp`** initializes from the server unit registry (`UnitDefinitions.max_hp_for_type`, 100 for both shipped types — never a wire-side literal) and **`has_attacked`** initializes `false` and resets for ALL units on every accepted world `end_turn`. There is still **no** `max_hp`, `next_unit_id`, movement points, or moved flag on the world path — world movement v1 has no movement budget ([MOVEMENT_RULES.md](MOVEMENT_RULES.md)). Remaining planned **additive** v3 fields land with their slices ([PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”): `cities` + `next_city_id` (N8a), city `current_project` (N8b), `next_unit_id` (N8c). **Alpha-store policy (locked):** shape-extending slices are not migrated — existing `world_map` matches are recreated after deployment (applies to N7g.1).
...
**`move_unit`**, **`end_turn`**, and (since **N7g.1**) **`attack_unit`** exist on the world path (wire shapes identical to the legacy actions below, all `schema_version` `1`). Known legacy action types **`found_city`** and **`set_city_production`** reject with **`unsupported_action_for_match_kind`** (their world equivalents are planned: `found_city` → N8a, `set_city_production` → N8b — [PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”; each keeps this rejection until its slice lands); unrecognized types reject with **`unknown_action_type`**. World action fields use **exact JSON integer** semantics: `schema_version` must be a non-boolean integer equal to `1` (boolean `true` and `1.0` reject as `unsupported_schema_version`); `actor_id`, `unit_id`, `attacker_id`, and `defender_id` must be non-boolean integers (`malformed_action` otherwise); coordinates were already exact-integer pairs.
...
- **Summary mode** (no selection): `actions` is exactly the submit-ready `end_turn`; `unit_summaries` lists the actor's units sorted by `unit_id` with `legal_action_count` equal to that unit's number of legal **attacks + moves** (N7g.2) — exactly the selected-unit row count; settlers stay move-only, and a unit whose `has_attacked` is `true` counts **0** (no attacks, no moves); `city_summaries` is `[]` (no world cities in N7).
