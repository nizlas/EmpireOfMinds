...
Only **`move_unit`** and **`end_turn`** exist on the world path (wire shapes identical to the legacy actions below, both `schema_version` `1`). Known legacy action types **`found_city`**, **`set_city_production`**, and **`attack_unit`** reject with **`unsupported_action_for_match_kind`**; unrecognized types reject with **`unknown_action_type`**. World action fields use **exact JSON integer** semantics: `schema_version` must be a non-boolean integer equal to `1` (boolean `true` and `1.0` reject as `unsupported_schema_version`); `actor_id` and `unit_id` must be non-boolean integers (`malformed_action` otherwise); coordinates were already exact-integer pairs.
