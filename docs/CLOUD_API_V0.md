...
**`units` (N7a; combat fields N7g.1):** deterministic, sorted ascending by **`id`**; each row is exactly `{"id", "owner_id", "position": [q, r], "type_id", "current_hp", "has_attacked"}` with `type_id` ∈ `settler` | `warrior`. **`current_hp`** initializes from the server unit registry (`UnitDefinitions.max_hp_for_type`, 100 for both shipped types — never a wire-side literal) and **`has_attacked`** initializes `false` and resets for ALL units on every accepted world `end_turn`. There is still **no** `max_hp`, `next_unit_id`, movement points, or moved flag on the world path — world movement v1 has no movement budget ([MOVEMENT_RULES.md](MOVEMENT_RULES.md)). **Additive v3 fields:** `cities` + `next_city_id` (**N8a**, implemented — empty `cities` and `next_city_id` `1` at create); city rows are `{"id", "owner_id", "position": [q, r], "name", "current_project"}` sorted by `id` (**N8b** additive `current_project`: `null` or `{"project_id", "progress", "cost"}`; newly founded cities start `null`). Remaining planned fields land with later slices ([PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”): `next_unit_id` (N8c). **Alpha-store policy (locked):** shape-extending slices are not migrated — existing `world_map` matches are recreated after deployment (applies to N7g.1, N8a, and N8b).
...
- **`end_turn`** validation order: `wrong_action_type → unsupported_schema_version → malformed_action → not_current_player`. It advances `turn_state` and (N7g.1) **clears `has_attacked` for all units** — still **no** production/food/science ticks, delivery, or movement refresh in N8b (production tick/delivery are N8c). The accepted event carries `turn_number_before` and `next_player_id` as in v2.
