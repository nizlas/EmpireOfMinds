...
- **Selected owned unit:** `actions` contains submit-ready **`attack_unit`** rows FIRST (N7g.2), then **`move_unit`** rows — both in the exact POST shapes and in canonical `DIRECTIONS` order (E, NE, NW, W, SW, SE) of the defender/destination tile, derived by filtering through the **same world validators the POST path uses** (N7g.1 attack validators; N7a move validators) against the resolved authoritative `WorldMap`. Attack rows are exactly `{"schema_version": 1, "action_type": "attack_unit", "actor_id", "attacker_id", "defender_id"}` for adjacent enemy **warriors** connected by a **smooth** edge; friendly units, settlers (either side), non-adjacent enemies, cliff edges, and missing edge records are excluded, and an attacker with `has_attacked` `true` advertises neither attacks nor moves — all by the same code that judges submitted actions, never a second legality implementation. Moves keep the N7a exclusions (cliffs, occupied tiles, missing edges, non-adjacent and off-map destinations). Every advertised action is submit-ready by construction.
