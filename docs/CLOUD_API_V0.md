...
**`move_unit`**, **`end_turn`**, (since **N7g.1**) **`attack_unit`**, and (since **N8a**) **`found_city`** exist on the world path (wire shapes identical to the legacy actions below, all `schema_version` `1`). Known legacy action type **`set_city_production`** rejects with **`unsupported_action_for_match_kind`** until N8b ([PHASE_PLAN.md](PHASE_PLAN.md) “Planned N7d–N8d”); unrecognized types reject with **`unknown_action_type`**. World action fields use **exact JSON integer** semantics: `schema_version` must be a non-boolean integer equal to `1` (boolean `true` and `1.0` reject as `unsupported_schema_version`); `actor_id`, `unit_id`, `attacker_id`, and `defender_id` must be non-boolean integers (`malformed_action` otherwise); coordinates were already exact-integer pairs. World **`found_city`** locked first-failure order: `wrong_action_type → unsupported_schema_version → malformed_action → not_current_player → unknown_unit → unit_not_owned_by_player → unit_cannot_found_city → unit_not_at_position → tile_already_has_city` (no water/territory/elevation reasons).
...
- **Selected owned city:** a known city owned by `actor_id` is a valid selection and returns **`actions`: `[]`** with **`selection_error`: `null`** until N8b production selection.
