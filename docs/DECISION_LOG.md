...
- **Proposed (pending Niclas review, not yet canon):** the ten proposed gameplay contracts listed in PHASE_PLAN “Proposed contracts pending review” — N7g attack gating via additive `has_attacked` (attack ends the unit's turn actions; move-then-attack allowed), melee requires a smooth edge, warrior-vs-warrior kept (warrior-vs-settler an open decision), no pre-N7g store migration; N8a cities never block movement / no min spacing; N8a shared-tile selection cycling; N8b no unlock gating (no world science in N8) and flat production **1**/city/turn with registry costs 2/2; N8c spawn on the owner's own `end_turn` (city tile, else first unoccupied smooth-adjacent in `DIRECTIONS` order, else deferred; project resets to `null`) — a flagged divergence from legacy delayed delivery; additive `next_city_id`/`next_unit_id` counters. Everything else reuses existing canonical contracts (Local Combat 0.1 math verbatim, legacy wire shapes, `CityProjectDefinitions`, city naming, legacy event names).
