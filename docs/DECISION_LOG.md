...
- **Still proposed (approve before the owning slice):** N8a cities never block movement / no minimum spacing; N8a shared-tile selection cycling; N8b no unlock gating (no world science in N8); N8b flat production **1**/city/turn with registry costs 2/2. Everything else reuses existing canonical contracts (Local Combat 0.1 math verbatim, legacy wire shapes, `CityProjectDefinitions`, city naming, legacy event names).
