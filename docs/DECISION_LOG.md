...
- **Server refactor:** canonical pure modules `server/app/domain/city_founding_rules.py` (Capital/Settlement-N naming, locked N8a founding decision chain + literal reasons, founding effect) and `city_production_rules.py` (locked N8b selection decision chain + literal reasons, canonical `{project_id, progress, cost}` project state, event progress representation, `FLAT_PRODUCTION_PER_CITY`); `world_production_rules.py` removed; `world_actions.py`/`world_legal_actions.py` reduced to thin wire/snapshot/WorldMap adapters; deprecated `actions/found_city.py` and `actions/set_city_production.py` delegate their shared rules (naming, event progress) to the core. External N8a/N8b behavior byte-identical (schemas, literal rejection reasons, first-failure ordering, legal-action ordering, snapshot/event shapes, revision/persistence/hash behavior, drift-500 before mutation).
...
- **Guards:** `server/tests/test_city_rules_canonical.py` — pure-rule behavior needs no transport/persistence; monkeypatched canonical rules provably drive world POST validation, apply, AND legal-actions derivation (same validators); AST dependency walls (canonical rules import no Scenario/HexMap/WorldMap/storage/API; world modules import no deprecated snapshot-v2 gameplay). `game/cloud/tests/test_world_city_ui_components.gd` — components render exclusively served rows; composition guard; world path never touches the deprecated local `production_tick`/`production_delivery` loop.
