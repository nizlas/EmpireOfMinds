...
class StubFamily:
	...
	const HEIGHT_HEAD_CANDIDATES := RealFamily.HEIGHT_HEAD_CANDIDATES
	const HEIGHT_FLOOR_CANDIDATES := RealFamily.HEIGHT_FLOOR_CANDIDATES
...
class StubFixture:
	...
	const RealFixture = preload(
		"res://presentation/equipment/uthana_warrior_hand_fixture.gd"
	)

	static func surface_for_side(side: String, character: Node, skeleton: Skeleton3D) -> Dictionary:
		return RealFixture.surface_for_side(side, character, skeleton)
...
func _run() -> void:
	...
	var left_ctx: Dictionary = await _spawn_side(host, "left", 0.35)
...
func _spawn_side(host: Node, side: String, t: float) -> Dictionary:
	var root := Node3D.new()
...
func _test_family_and_fixture(right_ctx: Dictionary, left_ctx: Dictionary) -> void:
	...
	var ls: Dictionary = Fixture.compile_left_from_mesh(left_ctx["character"], left_ctx["skeleton"])
	_check(bool(ls.get("compiled", false)), "left fixture compiled from mesh")
