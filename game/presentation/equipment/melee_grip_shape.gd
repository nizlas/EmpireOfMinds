# Local melee_1h grip-segment shape from a normalized weapon mesh.
# Axis and elliptical radii are measured, never a global default radius
# alone. Generic owner (A2.9); the A2 file is a compatibility shell.
extends RefCounted
...
## After normalize: primary_grip at weapon origin, +Y toward active end.
## Samples in the parent (socket) metric space so radii match the scaled weapon.
...
## Point on the elliptical cross-section in weapon-local XZ at angle (0 = +X).
...
## Signed radial gap from a weapon-local point to the ellipse surface.
