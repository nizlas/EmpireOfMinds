# A2 melee_1h asset compiler / normalizer (isolated feasibility).
# Dual grip-end hypotheses scored from geometry — never filename-only,
# never a handwritten transform for one club or one warrior.
extends RefCounted
