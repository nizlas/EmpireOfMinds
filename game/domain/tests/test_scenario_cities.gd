...
func _init() -> void:
	...
	_check(sc.tile_owner_city_id(HexCoordScript.new(1, -1)) == 1, "tile_owner_city_id default territory")
	_check(sc.city_owning_tile(HexCoordScript.new(1, -1)).id == 1, "city_owning_tile default territory")
	_check(sc.tile_is_owned(HexCoordScript.new(1, -1)), "tile_is_owned on city center")
	_check(sc.tile_owner_city_id(HexCoordScript.new(9, 9)) == -1, "tile_owner_city_id unowned")
	var tr = sc.tiles_owned_by_city(2)
	_check(tr.size() == 1 and tr[0].equals(HexCoordScript.new(1, 0)), "tiles_owned_by_city default is center")
