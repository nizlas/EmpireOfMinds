...
func _init() -> void:
	...
	_check(uw.max_hp == 100 and uw.current_hp == 100, "warrior full hp v0")
	...
	_check(us.max_hp == 100 and us.current_hp == 100, "settler full hp v0")
	var u_hurt = UnitScript.new(14, 0, HexCoordScript.new(0, 0), "warrior", 2, 55)
	_check(u_hurt.current_hp == 55, "explicit current_hp")
	var u0mp = UnitScript.new(12, 0, HexCoordScript.new(0, 0), "warrior", 0)
	_check(u0mp.remaining_movement == 0, "explicit 0 remaining")
	_check(UnitScript.new(13, 0, HexCoordScript.new(0, 0), "warrior", 99, 50).current_hp == 50, "hp not clamped by remaining clamp branch")
