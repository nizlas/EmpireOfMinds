"""N7b: read-only legal-actions enumeration for world_map matches.
...
Move rows are DERIVED, never re-implemented: candidates are constructed in
canonical DIRECTIONS order and filtered through the N7a world validators
(world_actions.validate_move_unit_pre_map + validate_move_unit_destination)
against the resolved authoritative WorldMap. Every returned action is
therefore submit-ready by construction — cliffs, occupied tiles, missing
edges, non-adjacent and off-map destinations are excluded by the same code
that judges POSTed moves. Returned move rows keep DIRECTIONS order.
...
def compute_world_legal_actions_payload(
    ...
    # Actor summary: submit-ready end_turn + per-unit move counts. Snapshot
    # units are already sorted ascending by id (N7a invariant).
    out["actions"] = [_end_turn_action(actor_id)]
    unit_summaries: list[dict[str, int]] = []
    for u in snap.get("units", []):
        if int(u["owner_id"]) != actor_id:
            continue
        n = len(_move_actions_for_unit(snap, world_map, actor_id, u))
