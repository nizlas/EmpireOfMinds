# Empire of Minds — 7-hex single-patch PBR ground + stone world-UV prototype (Blender only).
# Run from Blender Scripting workspace: Open → Run Script.
# Requires bpy (not available outside Blender).
#
# Builds on the visually approved Blender 5.1.2 ground-PBR UV baseline and adds a full
# stone PBR layer via the existing final stone splat weight. Ash remains procedural color.
# Ash PBR belongs in a separate prototype pass.
#
...
OUTPUT_BLEND_FILENAME = "terrain_prototype_7_hex_single_patch_pbr_ground_stone.blend"
OUTPUT_GLB_FILENAME = "terrain_prototype_7_hex_single_patch_pbr_ground_stone.glb"
...
# future layer work. Stone PBR is integrated here; ash PBR must remain a separate pass.

# Initial stone-PBR prototype values.
# Do not retune ground while evaluating stone.
...
PROCEDURAL_MATERIAL_NAME = "EOM_Terrain_PBR_Ground_Stone_Prototype"
SIDE_MATERIAL_NAME = "EOM_Terrain_Side_PBR_Ground_Stone_Prototype"
...
def _log(message: str) -> None:
    print(f"[EOM terrain stone PBR] {message}")
...
def _assert_debug_stage_routing_coverage() -> None:
    ...
    neutral_base_stages = frozenset({"stone_normal"})
    ...
    covered = base_color_stages | neutral_base_stages | pbr_override_stages
...
def _apply_debug_pbr_surface_overrides(
    ...
    *,
    ...
    stone_roughness_output: bpy.types.NodeSocket,
    ...
    if stage in {"ground_stone_pbr", "final"}:
        _unlink_socket_input(links, roughness_in)
        _unlink_socket_input(links, normal_in)
        ...
        if stage == "ground_stone_pbr":
            _log(
                "ground_stone_pbr debug: ground+stone PBR (ash bypassed on Base Color), "
...
def make_pbr_ground_stone_terrain_material(
    ...
    """Approved ground PBR UV baseline + stone PBR splat via existing final stone weight."""
    ...
    _log(f"normal strength={STONE_NORMAL_STRENGTH}")
    _log("uses existing final stone weight")
    _log("ash remains procedural")
    ...
    # Ash remains procedural; ground PBR normal/roughness still underlie ash until ash-PBR pass.
    ...
    normal_mix = _new_vector_mix_node(nodes, (420, -520))
    links.new(_require_output(ground_normal_map, "Normal"), _require_input(normal_mix, "A"))
    links.new(_require_output(stone_normal_map, "Normal"), _require_input(normal_mix, "B"))
    links.new(stone_factor_output, _require_input(normal_mix, "Factor"))
    blended_normal_output = _require_output(normal_mix, "Result")
    ...
    rough_mix = _new_float_mix_node(nodes, (1220, -120))
    links.new(ground_roughness_final, _require_input(rough_mix, "A"))
    links.new(stone_roughness_final, _require_input(rough_mix, "B"))
    links.new(stone_factor_output, _require_input(rough_mix, "Factor"))
    mixed_roughness_output = _require_output(rough_mix, "Result")
    ...
    _apply_debug_pbr_surface_overrides(
        ...
        stone_roughness_output=stone_roughness_final,
        ground_normal_map=ground_normal_map,
        stone_normal_map=stone_normal_map,
...
def _assert_shared_pbr_uv_chain(
    ...
    *,
    ...
    _log("shared by ground and stone albedo, normal, roughness")
...
def _assert_stone_pbr_splat_weights(
    *,
    mix_stone: ColorMixSockets,
    normal_mix: bpy.types.Node,
    rough_mix: bpy.types.Node,
    ...
    for label, socket in (
        ...
        ("normal_mix", _require_input(normal_mix, "Factor")),
        ("rough_mix", _require_input(rough_mix, "Factor")),
    ):
...
def _log_material_setup() -> None:
    _log("PBR ground + stone material created")
    ...
    _log("UV chain: ShaderNodeUVMap -> Ground UV Mapping -> ground/stone PBR textures")
    _log("Stone splat: existing final stone weight drives albedo, normal, and roughness")
    _log("Ash remains procedural (no ash PBR in this pass)")
...
def main() -> None:
    ...
    _log("generating 7-hex single-patch PBR ground + stone prototype…")
    ...
    procedural_material = make_pbr_ground_stone_terrain_material(
        ground_albedo_path,
        ground_normal_path,
        ground_roughness_path,
        stone_albedo_path,
