# Empire of Minds — TS-08 Stage 2 cut-domain thin-plate CG regeneration runner.
# Run: blender --background --python tools/blender/terrain/run_ts08_stage2_cut_domain_thin_plate_cg_regen.py
...
_RUNNER_NAME = "run_ts08_stage2_cut_domain_thin_plate_cg_regen.py"
_GENERATOR = "generate_ts08_stage2_cut_domain_thin_plate_cg.py"
    ...
    try:
        ...
        for text in bpy.data.texts:
            if not text.filepath:
                continue
            if Path(text.filepath).name not in {_RUNNER_NAME, _GENERATOR}:
                continue
            text_path = Path(bpy.path.abspath(text.filepath)).resolve()
            if _is_real_file(text_path):
...
def _resolve_terrain_tools_dir() -> tuple[Path, str]:
    ...
    raise FileNotFoundError(
        f"Could not locate tools/blender/terrain/{_GENERATOR}.\n"
...
GENERATOR = SCRIPT_DIR / _GENERATOR

spec = importlib.util.spec_from_file_location("eom_ts08_stage2", GENERATOR)
...
print(f"[{module.PROTOTYPE_ID}] dedicated TS-08 Stage 2 cut-domain thin-plate CG regeneration")
module.main()
