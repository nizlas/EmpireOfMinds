# Empire of Minds — 7-hex single-patch procedural material prototype (Blender only).
# Run from Blender Scripting workspace: Open → Run Script.
# Requires bpy (not available outside Blender).
#
# Geometry is intentionally kept identical to the approved
# single-patch terrain milestone. This script prototypes
# procedural terrain material blending only.
#
...
OUTPUT_BLEND_FILENAME = "terrain_prototype_7_hex_single_patch_material.blend"
OUTPUT_GLB_FILENAME = "terrain_prototype_7_hex_single_patch_material.glb"
...
PROCEDURAL_MATERIAL_NAME = "EOM_Terrain_Procedural_Prototype"
SIDE_MATERIAL_NAME = "EOM_Terrain_Side_Prototype"
...
def _log(message: str) -> None:
    print(f"[EOM terrain material] {message}")
...
def _apply_debug_base_color_link(
    ...
    *,
    ...
    stage_outputs = {
        "ground": _require_output(ground_rgb, "Color"),
...
def make_procedural_terrain_material() -> bpy.types.Material:
    """Three-layer procedural top material: ground, ash variation, cold stone on slope."""
    ...
    ground_rgb = _new_node(nodes, "ShaderNodeRGB")
    ground_rgb.location = (-640, 420)
    _require_output(ground_rgb, "Color").default_value = GROUND_BASE_COLOR
    ...
    links.new(_require_output(ground_rgb, "Color"), mix_ground_ash.color1)
    ...
    if USE_ROUGHNESS_VARIATION:
        ...
        _math_input(rough_mul, 1).default_value = ROUGHNESS_VARIATION_STRENGTH * 2.0

        rough_add = _new_node(nodes, "ShaderNodeMath")
        rough_add.location = (860, -120)
        rough_add.operation = "ADD"
        _math_input(rough_add, 1).default_value = BASE_ROUGHNESS
        ...
        links.new(_require_output(rough_mul, "Value"), _math_input(rough_add, 0))
        links.new(_require_output(rough_add, "Value"), _require_input(rough_clamp, "Value"))
        links.new(_require_output(rough_clamp, "Result"), roughness_out)
    else:
        roughness_out.default_value = BASE_ROUGHNESS
    ...
    if USE_PROCEDURAL_BUMP and BUMP_STRENGTH > 0.0:
        ...
        links.new(_require_output(bump, "Normal"), _require_input(principled, "Normal"))
...
def _log_material_setup() -> None:
    _log("Procedural material created")
    ...
    _log(f"Large noise scale: {LARGE_NOISE_SCALE}")
    _log(f"Fine noise scale: {FINE_NOISE_SCALE}")
    _log(f"Slope thresholds: start={SLOPE_BLEND_START}, end={SLOPE_BLEND_END}")
    _log(f"Stone breakup noise scale: {STONE_BREAKUP_NOISE_SCALE}")
    _log(f"Stone breakup strength: {STONE_BREAKUP_STRENGTH}")
    _log(f"Base roughness: {BASE_ROUGHNESS}")
...
def main() -> None:
    ...
    _log("generating 7-hex single-patch procedural material prototype…")
    ...
    procedural_material = make_procedural_terrain_material()
