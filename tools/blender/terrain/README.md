...
> **Current terrain reference (2026-08): the TS-08 chain.** The accepted terrain reference implementation is the **TS-08 cut-domain thin-plate chain** — Stage 0 (cut-lattice topology audit), Stage 1 (no-cut thin-plate CG), Stage 2 (cut-domain thin-plate CG), Stage 3a (basic cliff walls + stone wall material) — on the fixed `terrain_handdrawn_test_map_full_01` map. See [docs/TERRAIN_SURFACE_TARGET.md](../../../docs/TERRAIN_SURFACE_TARGET.md) (canonical model) and the "Current canonical model (TS-08)" section of [docs/TERRAIN_MODEL.md](../../../docs/TERRAIN_MODEL.md). Everything below in this README (7-hex prototypes, TS-03 baseline chain) is **earlier prototype history**: the material/PBR/world-UV work remains the approved **material** direction, but the **height/solve** models here are superseded by TS-08. The TS-07c (virtual rails), TS-07d (weighted curvature), and TS-08 Stage 3b (fitted cliff-panel) experiments are **superseded — do not build on them**. Blender is a development/reference implementation only; the approved runtime target is Godot-native terrain construction (see the "Fixed-grid Godot 3D terrain parity" milestone in [docs/PHASE_PLAN.md](../../../docs/PHASE_PLAN.md)).

Handdrawn full-map terrain prototypes (`generate_terrain_terrainmap_handdrawn_full_01.py` and `run_*_blend_regen.py` runners) are indexed in **[TERRAIN_PROTOTYPE_MANIFEST.md](TERRAIN_PROTOTYPE_MANIFEST.md)**. The TS-08 reference logical map lives at **`content/maps/reference/handdrawn_test_map_full_01.json`** ([docs/MAP_CONTENT.md](../../../docs/MAP_CONTENT.md)); Blender tooling loads it via `eom_map_content.py`. The approved smooth **TS-03** baseline (historical; superseded as height model by TS-08) is regenerated via **`generate_ts03_global_continuous_baseline.py`** (not the legacy cliff-cut `run_ts_variational_spline_blend_regen.py` path). Frozen baseline blends are audited read-only via `audit_ts03_baseline_blend.py`.
