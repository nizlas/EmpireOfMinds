# Empire of Minds — 7-hex single-patch PBR ground material prototype (Blender only).
...
# This script adds the first tileable PBR ground texture layer while ash/stone
# remain procedural colors with the existing mask/splatting logic.
#
...
OUTPUT_BLEND_FILENAME = "terrain_prototype_7_hex_single_patch_pbr_ground.blend"
OUTPUT_GLB_FILENAME = "terrain_prototype_7_hex_single_patch_pbr_ground.glb"
...
# Higher = more tile repeats; lower = larger, more stretched ground detail.
GROUND_TEXTURE_SCALE = 0.35
...
PROCEDURAL_MATERIAL_NAME = "EOM_Terrain_PBR_Ground_Prototype"
SIDE_MATERIAL_NAME = "EOM_Terrain_Side_PBR_Ground_Prototype"
...
def _log(message: str) -> None:
    print(f"[EOM terrain PBR ground] {message}")
...
def validate_material_params() -> None:
    ...
    if GROUND_TEXTURE_SCALE <= 0.0:
        raise ValueError(f"GROUND_TEXTURE_SCALE must be > 0, got {GROUND_TEXTURE_SCALE!r}")
...
def make_pbr_ground_terrain_material(
    ...
    ground_mapping = _new_node(nodes, "ShaderNodeMapping")
    ground_mapping.location = (-1300, -360)
    _require_input(ground_mapping, "Scale").default_value = (
    ...
    links.new(coord_output, _require_input(ground_mapping, "Vector"))
    mapped_vector = _require_output(mapping, "Vector")
    ground_mapped_vector = _require_output(ground_mapping, "Vector")
    ...
    links.new(ground_mapped_vector, _require_input(ground_albedo_tex, "Vector"))
    links.new(ground_mapped_vector, _require_input(ground_normal_tex, "Vector"))
    links.new(ground_mapped_vector, _require_input(ground_roughness_tex, "Vector"))
...
def _log_material_setup() -> None:
    _log("PBR ground material created")
    _log(f"Ground texture scale: {GROUND_TEXTURE_SCALE}")
...
def main() -> None:
    ...
    _log("generating 7-hex single-patch PBR ground material prototype…")
